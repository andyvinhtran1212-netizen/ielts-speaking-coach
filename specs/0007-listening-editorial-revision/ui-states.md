...
| Museum-map form | Keep source visual until reviewed variant is ready | Missing variant disables language control for that form | Reviewed language switches question, options and map labels/accessible text together | Invalid variant fails package validation; an unsignable asset returns a visible retry error without exposing answers | Existing signed private-asset guard | Preserve A–F/P–U anchors, contrast and zoom/alt text in both themes |
