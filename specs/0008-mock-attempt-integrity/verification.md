...
| FR-002 | kind=test; ref=backend/tests/test_mock_exam_workflow.py; gap=lookup failure and staging repair journey | PENDING |
| FR-003 | kind=test; ref=backend/tests/test_mock_exam_workflow.py; gap=exact-SHA staging and production journey | PENDING |
...
- No migration. Query the affected staging sitting and domain attempt by ID;
  ...
  an anomalous attempt. Compare immediate state with reload.
