...
- [x] T002 Add backend-only Writing pagination, row-backed Review eligibility, and bounded content-catalog routines plus canonical service queries. `depends: T001; owns: backend/migrations/297_admin_writing_queue_page.sql, backend/migrations/298_admin_mock_review_eligibility.sql, backend/migrations/299_admin_exam_content_page.sql, backend/migrations/300_admin_writing_queue_page_snapshot.sql, backend/migrations/301_admin_exam_content_page_snapshot.sql, backend/services/essay_service.py, backend/services/exam_content_service.py, backend/services/mock_exam_service.py`
...
- [ ] T006 Apply and verify migrations 297–301 on staging and production, complete independent review and exact-SHA staging evidence, then promote through the staging-first flow. `depends: T005`
