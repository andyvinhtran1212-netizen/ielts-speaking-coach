...
- **FR-004:** Live defaults to the first published, open exam in stable `created_at DESC, id DESC` order or a purposeful no-open-room action. Review defaults to the first eligible published exam in the same order: a sequential exam must be closed with canonical `active_section=done`; a retake exam may be open or closed but must have at least one sitting in `all_submitted`, `under_review`, or `reviewed` state with a persisted review in `queued`, `claimed`, `edited`, or `reviewed` status. The backend derives this eligibility from canonical sitting/review rows, never from the exam-level section or an optimistic client count, and declares the additive `review_eligible` boolean in the typed admin exam-list response without hiding existing exam fields. If no candidate exists, Review shows a purposeful no-actionable-review state. A valid explicit deep link is always honored even when that exam is not the default candidate; the Review rail keeps eligible open retakes visible. During backend/frontend deployment skew, missing retake eligibility is visibly unknown rather than described as no work. The Writing workspace does not display an exam rail that does not scope its data.
