...
Amendment 2026-09-23: Review eligibility is based on persisted actionable
review work for every exam mode. A completed sequential clock alone is not
evidence that work remains after all results are released.
...
- **FR-002:** The content-bank API and UI provide bounded search, preserve the existing `kind`, `course_level`, `cohort_id`, `exam_only`, and `is_public` filters, add defined dataset-wide attention filters, and support limit/offset pagination with an exact canonical total when every requested source succeeds. The page endpoint defaults to `limit=25`, accepts 1–100, and rejects larger values. Rows sort by kind, then case-insensitive code (falling back to title), then ID as a final tie-breaker; both the page endpoint and compatibility fallback apply this order before slicing. Search trims `q` to at most 100 characters and matches a case-insensitive literal substring of content ID, code, title, or course level; `%`, `_`, and punctuation have no wildcard meaning. The page endpoint and compatibility fallback use identical search fields and normalization. Every existing and new filter predicate is applied before counting and offsetting, including in compatibility mode. A partial content-source read carries an explicit incomplete-total signal and the UI never labels its surviving subtotal as canonical. The page response also includes the dataset-wide course-level catalog, scanned from the lightweight level column across all three sources independently of the current page and filters, with separate `levels_complete` and `failed_level_kinds` signals; a failed level scan never falsely downgrades an otherwise exact content total. Only the requested page is enriched and rendered, repeated row actions have contextual accessible names, and level edits require explicit save or cancel with canonical readback.
...
- **FR-004:** Live defaults to the first published, open exam in stable `created_at DESC, id DESC` order or a purposeful no-open-room action. Review defaults to the first eligible exam in the same order. Every eligible exam, including published sequential exams, must have at least one sitting in `all_submitted`, `under_review`, or `reviewed` state with a persisted review in `queued`, `claimed`, `edited`, or `reviewed` status. A published sequential exam must additionally be closed with canonical `active_section=done`; a published retake may be open or closed. Archived or draft exams with such persisted review work remain eligible regardless of exam mode or exam-level clock; changing exam publication status does not erase the work. An exam with no persisted review work or only released/void work is ineligible even if its section clock is `done`. The backend derives row-backed eligibility from canonical sitting/review rows, never from an optimistic client count, and declares the additive `review_eligible` boolean in the typed admin exam-list response without hiding existing exam fields. If no candidate exists, Review shows a purposeful no-actionable-review state. A valid explicit deep link is always honored even when that exam is not the default candidate; the Review rail keeps eligible open retakes and archived work visible. During backend/frontend deployment skew, missing row-backed eligibility is visibly unknown for every mode rather than described as no work. The Writing workspace does not display an exam rail that does not scope its data.
- **FR-005:** A new additive Mock Writing page endpoint applies student query, backend status, cohort, Mock scope, and overdue filters in PostgreSQL before limit/offset pagination, returns an exact total, and fetches and enriches only the page IDs in stable `created_at DESC, id DESC` order; service enrichment preserves the returned ID order. It defaults to `limit=25`, accepts 1–100, and rejects larger values. The existing array endpoint remains unchanged. Student query is trimmed to 100 characters, matches a case-insensitive literal substring of canonical student full name or student code, or a case-insensitive exact UUID; `%`, `_`, and punctuation are literal characters. `overdue=true` means status is not `delivered` and the earliest non-null deadline across duplicate/historical assignment rows is strictly earlier than PostgreSQL `now()`; the displayed deadline, database predicate, enrichment, and fallback use that same earliest-deadline rule without duplicating essays. A new-frontend/old-backend compatibility fallback applies unsupported active query and overdue predicates locally to the bounded legacy snapshot and is visibly incomplete rather than presented as canonical truth.
...
- **Then** the API total covers the complete matching dataset while the browser
  ...
  compatibility fallback. A course level found only beyond page 1 is still
  selectable; a failed level-column scan names the source and marks only the
  level catalog incomplete.
...
- **Given** two published open exams have the same `created_at`
- **When** the operator reloads Live without a deep link
- **Then** the one with the larger ID is selected consistently.

- **Given** Review has several published sequential exams: a closed `done`
  exam with queued review work, a newer closed `done` exam with only released
  results, an exam with no review rows, and one not at `done`
- **When** the operator opens Review without an exam deep link
- **Then** the closed `done` exam with queued work is selected, not the newer
  released-only or empty exam; if no actionable review rows remain the
  purposeful empty state is shown, while any valid explicit exam deep link
  remains selected.

- **Given** a published retake remains open while a student's non-released
  sitting has a queued review, and another retake has only released results
- **When** the operator opens Review without a deep link
- **Then** the actionable retake can be selected in stable newest-first order
  regardless of its exam-level `active_section`; the released-only retake is
  not eligible. The selected open retake remains visible in the Review rail.

- **Given** an operator archives a retake or sequential exam while a sitting
  still has a queued or claimed review, and another archived exam has only
  released results
- **When** the operator reloads Review without an exam deep link
- **Then** the archived actionable exam remains selectable and visible in the
  Review rail regardless of its exam-level clock; the released-only exam is
  not eligible. The same rule applies if the exam is returned to draft.
