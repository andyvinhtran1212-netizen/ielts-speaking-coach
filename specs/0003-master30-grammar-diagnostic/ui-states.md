...
| Readiness report | Report skeleton | Incomplete session links back safely | Priorities, evidence, and review routes match persisted snapshot, including after the submission deadline | Failed read distinguishes error from no report | Owner/admin only; active membership and publication still required; no cross-user evidence | Semantic sections, responsive cards, non-color status cues |
