...
| `/admin/classes/[cohortId]` | `/pages/admin/classes/index.html?cohort_id=…` remains homework/marking workspace and rollback target | `app/(authed-admin-classes)/admin/classes/[cohortId]/page.tsx` — native detail ownership 2026-08-12 | Admin | `tab=roster|progress|lessons` | AuthProvider + backend-owned `/auth/me` role guard; canonical cohort members/courses, progress and lessons reads; canonical reload after roster, class and lesson mutations | L | Native class profile, roster, four-skill progress and lesson timeline; homework/marking explicitly stays legacy until its independent batch |
