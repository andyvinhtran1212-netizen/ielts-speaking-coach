...
| `/practice/session` | NATIVE BOOTSTRAP + RECORDER / LEGACY ORCHESTRATION; admission vẫn `/pages/practice.html`, `route_ready=false` | `app/(authed-practice)/practice/session/page.tsx`; React sở hữu auth, session/question bootstrap và MediaRecorder lifecycle; SSR shell trích strict từ legacy; upload/grading/state machine vẫn `practice.js` | Student | `session_id` | AuthProvider + checked bootstrap handoff + native recorder controller + legacy orchestration; cùng backend/storage contract với legacy | XL | Stable implementation URL cho Gate E drill; chưa native player hoàn chỉnh, chưa cutover, không được dùng parity nhánh thiếu query làm player-ready evidence |
