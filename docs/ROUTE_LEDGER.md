...
| `/listening/test-dictation` | `/listening/dictation/session` is native dark route; admission remains legacy pending Gate E | `pages/listening-test-dictation.html` remains rollback target | Student | `test_id`, `section` | AuthProvider; server-canonical grade; account/test/section-scoped durable completion receipt in localStorage; audio playback | M | Native route owns picker, sentence timing/hints, word diff, canonical idempotent completion read-back, recovery and content flags |
