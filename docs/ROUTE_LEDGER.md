...
| `/practice/session` | NATIVE BOOTSTRAP + RECORDER + SUBMISSION + FULL-TEST STATE + PLAYER LIFECYCLE / LEGACY RENDERERS; admission vẫn `/pages/practice.html`, `route_ready=false` | `app/(authed-practice)/practice/session/page.tsx`; React sở hữu auth, session/question bootstrap, MediaRecorder, multipart/reconciliation, Full Test chain/retry/resume/finalize, state activation và cleanup timer/countdown/listener/speech/object URL; backend pin chain đúng part/cùng sitting, 9/1/5 câu và exact response coverage; SSR shell trích strict từ legacy; renderer/copy/feedback DOM vẫn `practice.js` | Student | `session_id` | AuthProvider + checked bootstrap + native player/recorder/submission/full-test controllers; receipt-safe canonical backend readback | XL | Stable implementation URL cho Gate E drill; chưa native React renderer, chưa browser/live drill hay cutover, không được dùng parity nhánh thiếu query làm player-ready evidence |
