...
| `/admin/classes/[cohortId]` | `/pages/admin/classes/index.html?cohort_id=…` remains the URL-only rollback target and retains the student-centric cross-assignment work-history view | `app/(authed-admin-classes)/admin/classes/[cohortId]/page.tsx` — native detail + homework + assignment-centric submission/marking ownership 2026-08-12 | Admin | `tab=roster|progress|lessons|homework`; optional `assignment_id` deep-link | AuthProvider + backend-owned `/auth/me` role guard; canonical cohort members/courses, progress, lessons, assignment reconciliation/catalog/action-log/tally/attempt-report/student-report/writing reads; canonical reload after every roster, class, lesson, assignment or return-work mutation | XL | Native class profile, roster, four-skill progress, lesson timeline, complete homework lifecycle and native per-assignment submission workspace: tally, effort/stalled/unfinished-writing, course answer report with canonical session/revision trail, writing review, Speaking artifact link and audited return-work. Legacy is deliberately not linked from the native flow; its URL remains rollback plus cross-assignment history until that separate student-centric view is ported. |
