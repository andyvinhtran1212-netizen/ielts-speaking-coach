...
| `/practice/session` | Stable Next implementation route; fresh runtime admissions land here, while `/pages/practice.html` remains the renderer-affinity rollback target for claimed Legacy sessions | `app/(authed-practice)/practice/session/page.tsx`; React sở hữu auth, atomic renderer claim, session/question bootstrap, MediaRecorder, multipart/reconciliation, Full Test chain/retry/resume/finalize, state activation, cleanup timer/countdown/listener/speech/object URL, static player DOM/handlers/SVG và structured view-model của header/loading/error/test progress/Part 1-3 prep/recording/processing/Part 2/assignment sheet/completion/feedback/pronunciation/test results; practice feedback có thể render tối đa hai curated vocab unit từ structured transcript evidence khi runtime flag bật; backend pin renderer theo session, chain đúng part/cùng sitting, 9/1/5 câu và exact response coverage; `practice.js` giữ legacy DOM fallback trên URL rollback | Student | `session_id` | AuthProvider + checked bootstrap + native player/recorder/submission/full-test controllers; receipt-safe canonical backend readback | XL | Floor `32043317793`, cutover `32045284608` và forward rollback `32047774312` đều pass. Operator-authorized early cutover 2026-08-24 admits fresh sessions to Next while preserving prior affinity; Gate E v13 remains 0/20, documented separately. |
| `/result` | `app/(authed-session-result)/result/page.tsx` — native React behavior 2026-08-09; `/pages/result.html` remains rollback target | `pages/result.html` (parity/rollback only) | Student | `id` (`session_id` accepted as compatibility alias) | AuthProvider, `/sessions/{id}`, signed audio, PDF export; requests/audio/blob URLs cleaned on unmount | L | Canonical persisted session result; fail-visible response lookup, sealed-mock state, grammar/pronunciation detail, curated unit từ persisted Speaking feedback, pending-vocab + KP widgets; Next player/history use canonical route while Legacy player stays on stable file URL |
...
| `/mock-exam` | `app/(authed-mock-exam)/mock-exam/page.tsx` — native React ownership 2026-08-17 | `pages/mock-exam.html` (parity/rollback only) | Student | `code` or `sitting` | AuthProvider; canonical sitting state + server clock; stable core-player admission for Listening/Reading; local + server Writing autosave; same-origin flush bridge; exact-payload submit reconciliation | XL | Sequential admin-gated LRW runner with retake mode, no early submit, canonical collection pauses, integrity signals and Speaking handoff. Legacy runner stays directly reachable until Gate E/F cutover and soak close. |
...
| `/vocabulary/learn` | `app/(authed-vocab-curated)/vocabulary/learn/page.tsx` — Vocab Curated V1 2026-08-27 | Không có legacy owner; tắt `vocab_units_read` để rollback về hub hiện tại | Student pilot (cohort + runtime default-deny) | none | AuthProvider, `/auth/me`, `/api/me/vocabulary/today`, `/api/vocabulary/pathways`; abort on unmount/account switch | M | Today/Paths cho learning unit tuyển chọn; route động `/vocabulary/learn/[unitSlug]` chạy vòng recall → control → transfer bằng server-graded attempt. Reference Wiki `/vocabulary` không đổi. |
...
| `/reading/exam/session` | Stable Next implementation route; fresh runtime admissions land here, while `/pages/reading-exam.html` remains the renderer-affinity rollback target for claimed Legacy attempts | `app/(authed-reading-player)/reading/exam/session/page.tsx` | Student for `test_id`; anonymous capability for `share` | `test_id` or `share`; optional `sitting_id`, `mock_embed`, `from`, `class_item` | AuthProvider, canonical Reading boot/attempt/answer/submit APIs, server-anchored timer, debounced retrying autosave, MockHook | XL | Native React player for all 16 Reading question types; restores canonical in-progress attempts and answers, submits the complete in-memory answer set, preserves password/share capability headers, and enters the native review route after submit. Floor `32060549833` attempt 3 on `7a6bdb9c`, cutover `32072244886` attempt 2 on `0599a8f3` and rollback `32076013600` attempt 2 on `14e38555` verified matching staging provenance. Operator-authorized early cutover 2026-08-24 admits fresh attempts to Next while existing claims remain sticky; Gate E v13 remains 0/20. |
...
| `/listening/general` | No legacy predecessor; package archive is the learner-visibility rollback | `app/(authed-listening)/listening/general/page.tsx` + shared `programme-library.tsx` — native React ownership 2026-09-21 | Student | filters: `all`, `new`, `in_progress`, `completed` | AuthProvider; programme-scoped lesson list and progress reads; package publication truth | M | General Listening programme library: 56 lessons and 143 report-only forms; checked/self-review mix, duration, replay/support metadata, explicit loading/empty/partial/error states and neutral non-CEFR wording. |
| `/listening/ielts` | Existing Quick/Skills/Mini/Full deep links remain direct rollback destinations; package archive removes only the new programme shelf | `app/(authed-listening)/listening/ielts/page.tsx` + shared `programme-library.tsx` — native React ownership 2026-09-21 | Student | filters: `all`, `new`, `in_progress`, `completed` | AuthProvider; programme-scoped lesson list and progress reads; existing mode routes remain independent | M | IELTS Practice report-only library followed by existing modes ordered by commitment: Quick, Skills, Mini, Full. Only eligible exam modes retain score/band wording. |
...
| `/listening/practice-run` | `/pages/listening-practice-run.html` giữ làm parity/rollback | `app/(authed-listening-practice-run)/listening/practice-run/page.tsx` + `practice-run-player.tsx` — native React ownership 2026-08-17 | Student | required exact `id` | AuthProvider; student-safe test + optional practice-window GET; resume-before-destructive-start; insert-once `/check`; submit + owner GET reconciliation; question-scoped audio | L | Luyện nhanh chấm từng câu. Điểm giữ câu trả lời đầu, đáp án chỉ mở sau hai lần sai đã xác nhận; mất ACK start/check/submit không được replay mù; payload grading leak làm trang fail closed; responsive và React-escaped. |
| `/listening/test/session` | Stable Next implementation route; fresh runtime admissions land here, while `/pages/listening-test.html` remains the renderer-affinity rollback target for claimed Legacy attempts | `app/(authed-listening-player)/listening/test/session/page.tsx` | Student; MockHook may seal access inside an active mock sitting | `test_id`; optional `attempt_id`, `sitting_id`, `mock_embed`, `from`, `class_item` | AuthProvider, canonical Listening test/attempt/answer/submit APIs, server-anchored audio position, debounced retrying autosave, MockHook | XL | Native React player for full, mini, drill, and practice Listening tests; restores canonical in-progress attempts and answers, blocks submit while any answer save remains unresolved, preserves single-play/no-scrub rules for full tests, and enters the native review route after submit. Dictation remains a separate post-result drill-down. Floor `32084645112` attempt 2, cutover `32093601359` attempt 2 and rollback `32095451591` attempt 1 verified matching staging provenance. Operator-authorized early cutover 2026-08-24 admits fresh attempts to Next while existing claims remain sticky; Gate E v13 remains 0/20. |
...
| `/listening/test-dictation` | `/listening/dictation/session` owns fresh admission; this Legacy URL remains the renderer-affinity rollback target | `pages/listening-test-dictation.html` retained until Gate F retirement | Student | `test_id`, `section` | Legacy auth/API bridge; canonical in-progress attempt + per-sentence save; audio playback | M | Claimed Legacy attempts remain resumable without changing renderer. |
| `/listening/dictation/session` | Stable Next implementation route after the core cutover; fresh runtime admissions land here | `app/(authed-listening-dictation)/listening/dictation/session/page.tsx` + `listening-dictation-session.tsx` | Student | `test_id`, `section` | AuthProvider; canonical in-progress attempt + atomic renderer claim; server-graded per-sentence upsert; account/test/section-scoped durable completion receipt; audio playback | M | Native route owns picker, sentence timing/hints, word diff, reload resume, canonical idempotent completion read-back, recovery and content flags; Legacy artifact remains available until Gate F closes. |
...
| Route Pattern | Aliases/Redirects | File | Auth | Query Params | Browser Deps | Complexity | Notes |
|---|---|---|---|---|---|---|---|
| `/advanced-vocabulary` | — (assignment-only core-30; không có legacy owner) | `app/(authed-advanced-vocab)/advanced-vocabulary/page.tsx` + `advanced-vocabulary-lesson.tsx` | Student with exact class assignment item | required `bank`, `item` | AuthProvider; `/api/advanced-vocab/*`; Kokoro MP3; assignment evidence ledger | XL | Self-paced Advanced Vocabulary ADV-T01…ADV-T30 thuộc Course 5: mỗi bài có 24 flashcards, hai practice stage, Reading cuộn độc lập, Controlled Rewrite gửi chấm batch một lần/unit, Listening, Writing reference và Speaking ladder/cue card. Hoàn tất cần sáu stage tương tác; không có điểm tổng. Writing/Speaking chỉ tham khảo và không tạo submission. |
| `/grammar-checkup` | — (MASTER30 native-only) | `app/(authed)/grammar-checkup/page.tsx` + `grammar-checkup.tsx` | Authenticated learner; optional class assignment | optional `assignment_item`, `session`, `view` | AuthProvider; `/api/grammar/diagnostics/*`; immutable release + exposure ledger | XL | Quick 28 câu hoặc Full 54 câu objective; server chọn và chấm; báo Grammar Readiness Profile, không quy đổi band IELTS. Productive tách khỏi auto-score. |
| `/grammar-checkup/review/[routeId]` | — (MASTER30 native-only) | `app/(authed)/grammar-checkup/review/[routeId]/page.tsx` | Owner of report session | required `session` | AuthProvider; report snapshot | M | Tuyến ôn cá nhân từ tối đa ba root priorities; không mở answer key. |
| `/admin/grammar-diagnostic` | — (MASTER30 native-only) | `app/(authed-admin-grammar)/admin/grammar-diagnostic/page.tsx` | Admin | required `session` | AdminAccessGate; `/admin/grammar-diagnostic/*` | M | Educator evidence view mở từ bài nộp trong lớp; điểm class ledger vẫn null. |
...
| Route Pattern | Aliases/Redirects | File | Auth | Query Params | Browser Deps | Complexity | Notes |
|---|---|---|---|---|---|---|---|
| `/admin/vocab` | `/pages/admin/vocab/index.html` remains rollback target | `app/(authed-admin-vocab)/admin/vocab/page.tsx` — native React ownership 2026-08-15 | Admin | none | AuthProvider + backend-owned `/auth/me` role guard | S | Native nine-workspace hub; canonical chrome/overview links use clean routes for Curated Editorial, Stats, Topics, Quick-Check Quiz, Quiz Analytics, D1 Curation and Lemma Overrides while remaining legacy consoles stay explicit until their own batches |
| `/admin/vocab/curated` | none — new Next-only operational surface | `app/(authed-admin-vocab)/admin/vocab/curated/page.tsx` + `admin-vocab-editorial.tsx` — native React ownership 2026-08-27 | Admin | none — status, page, headword/slug, reviewer inbox and selection are component-local | AuthProvider + backend-owned `/auth/me`; bounded canonical catalog/detail reads; strict response models; request/account freshness guards; canonical readback after review/publish/rollback | L | Master-detail Curated learning-unit review inbox with learner preview, immutable content/source/private-task diff, textual three-gate state, fresh validation requirement and explicit historical rollback confirmation. Inbox/search counts are explicitly page-local; server-side inbox filtering and deep links are deferred beyond the pilot. No legacy page is claimed because this workflow did not exist before V1. |
| `/admin/vocab/pilot-metrics` | none — new Next-only measurement surface | `app/(authed-admin-vocab)/admin/vocab/pilot-metrics/page.tsx` + `admin-vocab-pilot-metrics.tsx` — native React ownership 2026-08-27 | Admin | `days=30\|90\|180` | AuthProvider + backend-owned `/auth/me`; aggregate-only `/admin/vocabulary/pilot-metrics`; transactional cohort mutation + canonical readback; request freshness guards | M | Immediate/7-day/28-day outcomes and Speaking recommendation lifecycle. Delayed denominators include only closed observation windows; unavailable rates remain null and cohorts under 10 eligible unit-starts are visibly marked early. |
...
| `/admin/mock-exams/corrections` | — | `app/(authed-admin-mock-exams)/admin/mock-exams/corrections/page.tsx` | Admin | optional `learner_id`, `skill`, `class_assignment_id`, `mock_exam_id` filters submitted through the UI | AuthProvider + backend-owned `/auth/me`; canonical item attempts, correction sessions, append-only event timeline and content release-gate health | H | Internal correction-performance dashboard. Full explanation views remain distinct from correction output and mastery; malformed contracts fail visibly. |
