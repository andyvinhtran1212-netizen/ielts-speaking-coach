...
| `/practice/session` | NATIVE BOOTSTRAP / LEGACY PLAYER; admission vẫn `/pages/practice.html`, `route_ready=false` | `app/(authed-practice)/practice/session/page.tsx`; React sở hữu auth + tải/generate session/question, SSR shell trích strict từ legacy, recorder/grading vẫn `practice.js` | Student | `session_id` | AuthProvider + checked bootstrap handoff + legacy player state machine; cùng backend/storage/media contract với legacy | XL | Stable implementation URL cho Gate E drill; chưa native player, chưa cutover, không được dùng parity nhánh thiếu query làm player-ready evidence |
