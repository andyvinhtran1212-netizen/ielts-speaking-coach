...
| `/admin/vocab/curated` | none — new Next-only operational surface | `app/(authed-admin-vocab)/admin/vocab/curated/page.tsx` + `admin-vocab-editorial.tsx` — native React ownership 2026-08-27 | Admin | catalog `status=draft\|published\|archived`; local headword/slug and reviewer-inbox filters | AuthProvider + backend-owned `/auth/me`; bounded canonical catalog/detail reads; strict response models; request/account freshness guards; canonical readback after review/publish/rollback | L | Master-detail Curated learning-unit review inbox with learner preview, immutable content/source/private-task diff, textual three-gate state, fresh validation requirement and explicit historical rollback confirmation. No legacy page is claimed because this workflow did not exist before V1. |
