...
| `/practice/session` | DARK ROUTE BRIDGE; admission vẫn `/pages/practice.html`, `route_ready=false` | `app/(authed-practice)/practice/session/page.tsx`; SSR shell trích strict từ legacy, behavior vẫn `practice.js` | Student | `session_id` | AuthProvider + legacy state machine; cùng backend/storage/media contract với legacy | XL | Stable implementation URL cho Gate E drill; chưa native, chưa cutover, không được dùng parity nhánh thiếu query làm player-ready evidence |
