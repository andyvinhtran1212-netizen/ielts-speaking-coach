...
| `/practice/session` | NATIVE BOOTSTRAP + RECORDER + SUBMISSION + FULL-TEST STATE / LEGACY PLAYER UI; admission vẫn `/pages/practice.html`, `route_ready=false` | `app/(authed-practice)/practice/session/page.tsx`; React sở hữu auth, session/question bootstrap, MediaRecorder, multipart/reconciliation và Full Test chain/retry/resume/finalize; backend pin chain đúng part/cùng sitting, 9/1/5 câu và exact response coverage; SSR shell trích strict từ legacy; DOM state/effects + feedback rendering vẫn `practice.js` | Student | `session_id` | AuthProvider + checked bootstrap + native recorder/submission/full-test controllers; receipt-safe canonical backend readback | XL | Stable implementation URL cho Gate E drill; chưa native player UI hoàn chỉnh, chưa cutover, không được dùng parity nhánh thiếu query làm player-ready evidence |
