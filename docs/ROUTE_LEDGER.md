...
| `/admin/classes/[cohortId]` | `/pages/admin/classes/index.html?cohort_id=…` remains marking workspace and rollback target | `app/(authed-admin-classes)/admin/classes/[cohortId]/page.tsx` — native detail + homework assignment lifecycle ownership 2026-08-12 | Admin | `tab=roster|progress|lessons|homework` | AuthProvider + backend-owned `/auth/me` role guard; canonical cohort members/courses, progress, lessons, assignment reconciliation/catalog/action-log reads; canonical reload after every roster, class, lesson or assignment mutation | XL | Native class profile, roster, four-skill progress, lesson timeline and complete homework assignment lifecycle (class/subset recipients, VN due time, mastery config, backfill, archive/reopen/delete, compare-and-confirm due changes); submission tally/effort/individual marking explicitly stays at a deep-linked legacy seam until its independent batch |
