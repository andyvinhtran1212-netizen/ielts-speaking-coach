...
3. Node runtime (không Edge); Vercel function region đặt GẦN RAILWAY backend. **Region inventory (đo 2026-07-14 — ĐÓNG mục mở này):** Railway backend (prod + staging) = **Singapore `sin1`** (`x-railway-edge: sin1`); Vercel edge PoP hiện = `hkg1` (Hong Kong); Vercel function region trước đó **UNSET** → mặc định về US-East, tức mỗi uncached SSR fetch grammar là vòng xuyên Thái Bình Dương. **Set `regions: ["sin1"]` trong `vercel.json`** (project-wide function region) để mọi Vercel function chạy ở Singapore — fetch backend intra-region. LƯU Ý (review #757): `preferredRegion` (route segment export) CHỈ hiệu lực với `runtime='edge'`; grammar route là Node (default) + Cache Components nên phải dùng `vercel.json regions` chứ không phải `preferredRegion` (Edge-only, bị bỏ qua trên Node). sin1 tối ưu cho cả backend LẪN user (Vietnam ≈ 30ms), nên project-wide hợp lý.
