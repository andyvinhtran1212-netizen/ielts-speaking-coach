...
3. Node runtime (không Edge); Vercel function region đặt GẦN RAILWAY backend. **Region inventory (đo 2026-07-14 — ĐÓNG mục mở này):** Railway backend (prod + staging) = **Singapore `sin1`** (`x-railway-edge: sin1`); Vercel edge PoP hiện = `hkg1` (Hong Kong); Vercel function region trước đó **UNSET** → mặc định về US-East, tức mỗi uncached SSR fetch grammar là vòng xuyên Thái Bình Dương. **Đã pin `export const preferredRegion = 'sin1'` trên grammar route** để fetch backend là intra-region. Chỉ grammar SSR-fetch backend (landing/profile fetch client-side) nên pin ở route đó, không global.
