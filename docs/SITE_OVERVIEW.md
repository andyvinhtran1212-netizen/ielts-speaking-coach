...
| `pages/mock-exam.html` | student | 4-skill mock **runner** (SEQUENTIAL, admin-gated). `?code=` opens/resumes a sitting; there is no student "Start" — the admin opens Listening → Reading → Writing one at a time (`POST /admin/mock-exams/{id}/advance`), and the runner polls + shows exactly ONE section under its own shared-clock countdown (Listening = audio length + buffer, Reading/Writing = admin-set minutes). No early manual submit — each section auto-submits when its own clock hits 0; between sections the student sees a waiting room. Reading/Listening are the existing runners embedded as `mock_embed` iframes (auto-start, chrome hidden, autosaved); Writing is a native 2-tab step. Scores withheld until release. |
...
| `/admin/listening` (native content inventory) · `/admin/listening/content/[contentId]` (native content detail/status) · `/admin/listening/content/[contentId]/edit` (native versioned metadata editor) · `/admin/listening/segments` (native multi-block Dictation authoring) · `/admin/listening/tests` (native test inventory/status/exam scope) · `/admin/listening/tests/[testId]` (native audio/map/publication workspace) · `/admin/listening/attempts` (native learner-attempt evidence) · `/admin/listening/dictation` (native dictation aggregate/session evidence) · corresponding `pages/admin/listening/*.html` rollback pages · `pages/admin/listening/import-fulltest.html` · `pages/admin/listening/import-drills.html` · `pages/admin/listening/audit.html` · `pages/admin/listening/audit-detail.html` · `pages/admin/listening/segments.html` · `pages/admin/listening/content-meta.html` · `pages/admin/listening/gist.html` · `pages/admin/listening/tf.html` · `pages/admin/listening/mcq.html` · `pages/admin/listening/tests-detail.html` | admin | Listening operations (`/admin/listening/*`): native inventories, canonical content/test detail, versioned metadata editing, exact-block Dictation segment authoring, learner attempts and dictation evidence own clean routes. Metadata writes send only field deltas, support explicit nullable-field clearing and require canonical GET reconciliation. The segment editor preserves distinct `order_num` blocks, saves by exact exercise/version and never replays an ambiguous POST. Attempt and dictation history distinguish association lookup failure from missing records; dictation aggregates page the complete filtered scope and detail compares each reference sentence with learner input. Test detail reconciles mode, audio, assemble, maps, lifecycle and destructive actions with backend truth. Gist/T-F/MCQ editors and import/audit flows remain legacy workspaces until migrated. Includes **4-file full-test pack import** (`import-fulltest.html`, #408) + skill-drill import, build per-type exercises + Cambridge tests. (ElevenLabs assembly remains active inside test detail; standalone audio cutter / convert DOCX were decommissioned 2026-07-17 — usage audit.) Deep ref: [`listening-architecture.md`](listening-architecture.md). |
