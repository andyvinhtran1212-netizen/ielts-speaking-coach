...
> 1. **Tĩnh (CI, bắt buộc):** `python scripts/validate_grammar_quiz_bank.py ../docs/grammar-quiz-banks/*.md` — cấu trúc + mastery + `content_lint` (đáp án tự-mâu-thuẫn, accept khó gõ, `/` trong accept, "để trống/ø" trong prompt gap_text, accept[0] > 3 từ; audit 2026-07-17: ngoặc-trong-prompt/hint chứa nguyên văn đáp án — trừ cloze `____ (make)` và hint lựa chọn "chọn X hoặc Y"; instruction nhúng "— write…"/ngoặc hướng dẫn cuối câu).
