...
| **4** | **Nguồn chân lý (single source of truth)** là Claude Design hay `tokens.css`? | ~~Claude Design là nguồn~~ → **ĐÃ ĐẢO 2026-07-25: `tokens.css` (code) là nguồn, project là bản sync LÊN.** Một chiều **code → project**. | Khuyến nghị ban đầu ngược lại, nhưng thực tế 24/07 cho thấy project là ảnh chụp 05-09 còn code đã tiến hoá thật vượt qua nó (6 kỹ năng, token chrome/width, dark navy). Đổi token thì đổi trong code trước rồi sync lên. Xem `DRIFT_REPORT.md` §F. |
