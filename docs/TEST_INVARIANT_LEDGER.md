...
| `listening-dictation-next-behavior.test.mjs` + `verify-listening-dictation-session-flow.mjs` + `test_listening_test_dictation.py` + migration 210 | ✓ | `/listening/dictation/session` | Exact test/section identity; safe audio URL; multi-section picker; timed/free-scrub audio; server-canonical per-sentence grade; hostile text escaping; account/test/section durable receipt; payload fingerprint; commit-with-lost-ACK read-back; exact full-section coverage; canonical summary; per-sentence flags; responsive containment | backend + migration + model/source + fixture-backed browser | native dark-route owner; runtime admission remains legacy until Listening Dictation Gate E matrix passes |
