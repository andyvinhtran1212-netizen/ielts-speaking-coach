...
| `admin-listening-audit.test.mjs` | ✓ dashboard replacement: `admin-listening-audit-next-behavior.test.mjs` + fixture browser verifier; legacy detail characterization retained | `frontend/app/(authed-admin-listening)/admin/listening/audit/`, `frontend/lib/admin-listening-audit-model.mjs`, legacy audit-detail | Audit dashboard: stable all-test inventory, bounded canonical GET scan, live-vs-saved truth, lookup failure, URL filters and responsive cards | model-contract + fixture-browser + rollback characterization | native dashboard invariant active; retain detail assertions until audit-detail migration |
