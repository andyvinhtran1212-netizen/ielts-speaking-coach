...
active-session drill của Listening Dictation và
...
| Sticky active-session hoặc drain strategy đã drill | **PARTIAL** | Stable-player-URL admission mechanism đã chọn; launcher dùng runtime endpoint no-store. Speaking, Reading và Listening test đều đã đủ three-phase trên matching provenance. Listening rollback `32095451591` attempt 1 verify fresh Legacy attempt `153ef00d…` và sticky prior Next attempt `6f67fa07…`, `rollback_mode=forward-revert`. | Listening Dictation còn thiếu three-phase drill; Safari/iOS thật vẫn thiếu nên hàng cross-core này còn PARTIAL. |
