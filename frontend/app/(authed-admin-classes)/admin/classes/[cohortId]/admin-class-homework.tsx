    ...
    <section id="acx-panel-homework" className="acd-workspace ach-workspace" role="tabpanel" aria-labelledby="acx-tab-homework">
      ...
      <Dialog open={Boolean(editor)} title="Giao bài mới" description="Chọn nội dung, người nhận và hạn trước khi ghi vào sổ bài giao." busy={busy} onClose={closeEditor} actions={<><button className="adm-btn-secondary" type="button" onClick={closeEditor} disabled={busy}>Hủy</button><button className="adm-btn-primary" type="submit" form="ach-homework-form" disabled={busy || catalogLoading}>{busy ? 'Đang giao…' : editor?.recipientScope === 'subset' ? `Giao cho ${editor.studentIds.length} học viên` : 'Giao cho cả lớp'}</button></>}>
        <form id="ach-homework-form" className="acd-form ach-form" onSubmit={submitHomework}>
          ...
          {(editor?.skill === 'reading' || editor?.skill === 'listening') && <p className="acd-muted">Đề draft hoặc chưa gán lớp được giữ khóa tại đây. <a href="/admin/mock-exams">Mở kho đề để kiểm tra, gán lớp và publish</a>.</p>}
          {(editor?.skill === 'reading' || editor?.skill === 'listening') && <div className="ach-correction-policy">
            ...
            <fieldset className="ach-policy-group">
              ...
              <label className={`ach-policy-card${selectedCatalogItem && !selectedCatalogItem.explanation_ready ? ' is-disabled' : ''}`}><input type="radio" name="ach-explanation-mode" checked={editor.webExplanationMode === 'immediate_after_capture'} disabled={Boolean(selectedCatalogItem && !selectedCatalogItem.explanation_ready)} onChange={() => setEditor({ ...editor, webExplanationMode: 'immediate_after_capture', postTestCaptureRequired: true, error: '' })} /><span><b>Tự mở sau tự đánh giá ngắn</b><small>Phù hợp bài practice: thu confidence rồi tự trả đáp án và lời giải theo từng bước.</small></span></label>
            ...
            {selectedCatalogItem && selectedCatalogItem.explanation_state !== 'ready' && editor.webExplanationMode !== 'disabled' && <div className="acd-warning">Web explanation chưa qua đủ gate ({selectedCatalogItem.explanation_ready_count ?? '—'}/{selectedCatalogItem.explanation_count ?? '—'} câu sẵn sàng). Chế độ giáo viên mở sau vẫn giao được, nhưng chỉ phát khi toàn bộ 40 câu đã duyệt.</div>}
