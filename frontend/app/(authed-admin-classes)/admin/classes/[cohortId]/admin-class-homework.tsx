...
export function AdminClassHomework({ cohortId, members, refreshKey, onMutation, onOpenSubmissions, onOpenAssignmentMissing, openAssignmentId }: {
  cohortId: string;
  members: Member[];
  refreshKey: number;
  onMutation: () => void;
  onOpenSubmissions?: (assignment: ClassAssignment) => void;
  onOpenAssignmentMissing?: () => void;
  ...
  const loadCatalog = useCallback(async (draft: HomeworkDraft) => {
    ...
    try {
      ...
      const normalized = normalizeCatalog(value, kind, draft.skill, cohortId) as CatalogOption[] | null;
  ...
  useEffect(() => {
    ...
    const requested = assignments.find((assignment) => assignment.id === openAssignmentId);
    if (requested) onOpenSubmissions(requested);
    else { setBanner({ kind: 'error', text: 'Không tìm thấy bài giao cần mở trong lớp này.' }); onOpenAssignmentMissing?.(); }
  }, [assignments, onOpenSubmissions, onOpenAssignmentMissing, openAssignmentId]);
  ...
  return (
    <section id="acx-panel-homework" className="acd-workspace ach-workspace" role="tabpanel" aria-labelledby="acx-tab-homework">
      ...
      {loading && !assignments ? <div className="acd-state"><strong>Đang đối chiếu bài giao…</strong><span>Hệ thống đang sửa sổ cái từ các lượt hoàn thành trước khi hiển thị.</span></div> : assignments && !assignments.length ? <div className="acd-state"><strong>Chưa giao bài nào</strong><span>Giao bài đầu tiên để học viên thấy công việc trong My Class.</span></div> : assignments && !visible.length ? <div className="acd-state"><strong>Không có bài khớp bộ lọc</strong><span>Thử đổi từ khoá hoặc trạng thái.</span></div> : assignments ? <div className="acd-table-scroll" tabIndex={0} role="region" aria-label="Bảng bài tập của lớp, có thể cuộn ngang"><table className="acd-table ach-table"><thead><tr><th>Bài giao</th><th>Hạn nộp</th><th>Tình hình nộp</th><th><span className="sr-only">Thao tác</span></th></tr></thead><tbody>{visible.map((assignment) => { const state = statusOf(assignment); const progress = assignment.progress; return <tr key={assignment.id}><td><div className="ach-title"><strong>{assignment.title}</strong><span className={`adm-status-pill ${state.tone}`}>{state.label}</span></div><small>{assignmentSub(assignment)} · {assignment.recipient_scope === 'subset' ? 'Theo nhóm' : 'Cả lớp'}</small></td><td className="ach-due-cell"><strong>{formatDue(assignment.due_at)}</strong><small>Giờ Việt Nam</small></td><td className="ach-progress-cell">{progress ? <div className="ach-progress"><strong>{progress.submitted}/{progress.assigned} đã nộp</strong>{progress.late > 0 && <span>{progress.late} nộp trễ</span>}{progress.missing > 0 && <span className="is-warning">{progress.missing} chưa nộp, đã quá hạn</span>}{progress.no_account > 0 && <span>{progress.no_account} chưa kích hoạt</span>}</div> : <span className="acd-unknown">Không đọc được</span>}</td><td><div className="ach-actions"><button className="adm-btn-primary adm-btn-sm" type="button" onClick={() => onOpenSubmissions?.(assignment)}>Nhận bài</button><button className="adm-btn-secondary adm-btn-sm" type="button" onClick={() => setBackfill({ assignment, studentIds: [], error: '' })} disabled={busy}>Bù học viên</button><button className="adm-btn-secondary adm-btn-sm" type="button" onClick={() => openDue(assignment)} disabled={busy}>Đổi hạn</button>{assignment.status === 'archived' ? <button className="adm-btn-secondary adm-btn-sm" type="button" onClick={() => void setAssignmentStatus(assignment, 'published')} disabled={busy}>Mở lại</button> : progress == null || progress.submitted > 0 || assignment.timed_started_at ? <button className="adm-btn-secondary adm-btn-sm" type="button" onClick={() => void setAssignmentStatus(assignment, 'archived')} disabled={busy}>Đóng bài</button> : <button className="adm-btn-secondary adm-btn-sm" type="button" onClick={() => setConfirm({ kind: 'delete', assignment })} disabled={busy}>Xoá</button>}</div></td></tr>; })}</tbody></table></div> : null}
        ...
        <form id="ach-homework-form" className="acd-form ach-form" onSubmit={submitHomework}>
          ...
          {(editor?.skill === 'reading' || editor?.skill === 'listening') && <div className="ach-correction-policy">
            <fieldset className="ach-policy-group">
              ...
              <label className="ach-policy-card"><input type="radio" name="ach-delivery-mode" checked={editor.deliveryMode === 'standard'} onChange={() => setEditor({ ...editor, deliveryMode: 'standard', error: '' })} /><span><b>Đề đã mở trong thư viện chung</b><small>Bài giao trỏ tới đề mà học viên vốn đã có quyền luyện. Không cấp quyền cho đề Cambridge đang khóa.</small></span></label>
              <label className="ach-policy-card"><input type="radio" name="ach-delivery-mode" checked={editor.deliveryMode === 'assigned_practice'} onChange={() => setEditor({ ...editor, deliveryMode: 'assigned_practice', error: '' })} /><span><b>Chỉ học viên được giao</b><small>Cấp quyền đúng cho bài giao này; đề vẫn không public. Dùng cho Cambridge và đề exam_only.</small></span></label>
            </fieldset>
            <fieldset className="ach-policy-group">
              <legend>Khi nào mở lời giải tương tác?</legend>
              <label className="ach-policy-card"><input type="radio" name="ach-explanation-mode" checked={editor.webExplanationMode === 'disabled'} onChange={() => setEditor({ ...editor, webExplanationMode: 'disabled', error: '' })} /><span><b>Không dùng lời giải tương tác</b><small>Học viên nhận kết quả và phần chữa bài thường; web explanation object không được mở.</small></span></label>
              <label className="ach-policy-card"><input type="radio" name="ach-explanation-mode" checked={editor.webExplanationMode === 'immediate_after_capture'} onChange={() => setEditor({ ...editor, webExplanationMode: 'immediate_after_capture', postTestCaptureRequired: true, error: '' })} /><span><b>Tự mở sau tự đánh giá ngắn</b><small>Bật lựa chọn này đồng thời xác nhận duyệt 40 lời giải của đề; học viên nhận lời giải theo từng bước.</small></span></label>
              <label className="ach-policy-card"><input type="radio" name="ach-explanation-mode" checked={editor.webExplanationMode === 'admin_release'} onChange={() => setEditor({ ...editor, webExplanationMode: 'admin_release', postTestCaptureRequired: true, error: '' })} /><span><b>Giáo viên mở lời giải sau</b><small>Điểm và đáp án thường không bị giữ bởi lựa chọn này; chỉ web explanation chờ giáo viên duyệt.</small></span></label>
            </fieldset>
            {selectedCatalogItem && selectedCatalogItem.explanation_state !== 'ready' && editor.webExplanationMode !== 'disabled' && <div className="acd-warning">Đang chờ admin duyệt ({selectedCatalogItem.explanation_ready_count ?? '—'}/{selectedCatalogItem.explanation_count ?? '—'} câu). Khi bấm giao, hệ thống sẽ xác nhận duyệt đúng 40 lời giải của đề này; đề thiếu object hoặc có matcher/serving blocker vẫn bị chặn.</div>}
            <label className={`ach-confidence-option${editor.webExplanationMode === 'disabled' ? ' is-disabled' : ''}`}><input type="checkbox" checked={editor.webExplanationMode !== 'disabled' && editor.postTestCaptureRequired} disabled={editor.webExplanationMode === 'disabled'} onChange={(event) => setEditor({ ...editor, postTestCaptureRequired: event.target.checked, error: '' })} /><span><b>Thu mức chắc chắn 1–5 trước khi hiện đáp án</b><small>Không ảnh hưởng điểm. Dùng để phát hiện câu đoán đúng và lỗi hiểu sai nhưng confidence cao.</small></span></label>
            <div className="ach-policy-summary" role="status"><b>Học viên sẽ trải qua:</b> {editor.webExplanationMode === 'disabled'
              ? 'Nộp bài → xem kết quả và phần chữa bài thường.'
              : editor.webExplanationMode === 'immediate_after_capture'
                ? `${editor.postTestCaptureRequired ? 'Nộp bài → chọn confidence → ' : 'Nộp bài → '}xem kết quả và tự mở lời giải tương tác.`
                : `${editor.postTestCaptureRequired ? 'Nộp bài → chọn confidence → ' : 'Nộp bài → '}xem kết quả thường; web explanation mở khi giáo viên duyệt.`}</div>
          </div>}
          ...
          {editor?.skill === 'course' && selectedCatalogItem?.runtime === 'advanced_vocab' ? <div className="acd-warning"><strong>Bài self-paced không chấm điểm mặc định.</strong> Hệ thống xác nhận hoàn tất khi học viên xong 6 phần bắt buộc. Writing và Speaking chỉ để tham khảo; muốn chấm Writing, giáo viên giao bài Writing riêng.</div> : editor?.skill === 'course' && <>
            <fieldset className="ach-policy-group">
              ...
              <label className="ach-policy-card"><input type="radio" name="ach-completion-mode" checked={editor.completionMode === 'single_attempt'} onChange={() => setEditor({ ...editor, completionMode: 'single_attempt', error: '' })} /><span><b>Một lượt — hiện kết quả sau khi nộp</b><small>Trong lúc làm không lộ đúng/sai hay giải thích. Nộp xong hiện điểm %, đáp án và lời giải; không có nút làm lại.</small></span></label>
            </fieldset>
