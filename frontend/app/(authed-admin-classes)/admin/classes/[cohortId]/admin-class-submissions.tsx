...
function AdvancedAnswerEvidence({ rows, label }: { rows: { id: string; submitted_answer: unknown; is_correct: boolean }[]; label: string }) {
  ...
  return <section id="acx-panel-homework" className="acd-workspace acs-workspace" role="tabpanel" aria-labelledby="acx-tab-homework">
    ...
    {!loading && !error && view === 'tally' && tally && <div id="acs-panel-tally" role="tabpanel" aria-labelledby="acs-tab-tally" className="acs-tally">
      {bankId ? <section className="acs-outcomes" aria-label="Tổng kết kết quả học tập">
        <article data-kind="passed"><strong>{resultOnly ? tally.counts.completed : tally.counts.passed}</strong><span>{isAdvancedVocabulary || resultOnly ? 'Đã hoàn tất' : 'Đã đạt'}</span></article>
        {!isAdvancedVocabulary && !resultOnly && <article data-kind="timeout"><strong>{tally.counts.timed_out}</strong><span>Đã hết giờ</span></article>}
        {!isAdvancedVocabulary && !resultOnly && <article data-kind="near"><strong>{tally.counts.near_pass}</strong><span>Gần đạt · Revision</span></article>}
        {!isAdvancedVocabulary && !resultOnly && <article data-kind="retry"><strong>{tally.counts.retry_full}</strong><span>Làm lại toàn bài</span></article>}
