...
export function AdminClassSubmissions({ cohortId, assignment, initialStudent = null, backLabel = 'Quay lại danh sách bài tập', memberNames, memberUserIds, memberNamesAvailable, onBack, onMutation }: SubmissionWorkspaceProps) {
  ...
  return <section id="acx-panel-homework" className="acd-workspace acs-workspace" role="tabpanel" aria-labelledby="acx-tab-homework">
    ...
    {!loading && !error && view === 'effort' && effort && <div id="acs-panel-effort" role="tabpanel" aria-labelledby="acs-tab-effort" className="acs-effort">{effort.stale && <div className="acd-warning">Chưa đọc đủ dữ liệu bài làm; vài dòng có thể thiếu.</div>}{!effort.students.length ? <div className="acd-state"><strong>{effort.stale ? 'Chưa đọc được danh sách lượt làm' : 'Chưa có ai mở bài này'}</strong><span>{effort.stale ? 'Không kết luận đây là danh sách rỗng; hãy tải lại.' : 'Bảng sẽ xuất hiện khi học viên bắt đầu làm bài.'}</span></div> : <><div className="acd-table-scroll" tabIndex={0}><table className="acd-table"><thead><tr><th>Học viên</th><th>Tình trạng</th><th>Hoàn thành</th><th>Lượt đã chấm</th><th>Điểm tổng gần nhất</th><th>Tổng thời gian</th><th><span className="sr-only">Thao tác</span></th></tr></thead><tbody>{effort.students.map((row) => { const rosterHasStudent = row.student_id != null && Object.prototype.hasOwnProperty.call(memberNames, row.student_id); const name = row.student_id ? memberNames[row.student_id] || (memberNamesAvailable ? 'Học viên đã rời lớp' : 'Không đọc được tên học viên') : 'Học viên đã rời lớp'; const tallyRow = row.student_id ? tally?.students.find((item) => item.student_id === row.student_id) : null; const unactivated = row.student_id != null && memberNamesAvailable && rosterHasStudent && memberUserIds[row.student_id] == null; return <tr key={row.student_id || row.user_id}><td>{name}{unactivated && <small className="acs-account-note">Chưa kích hoạt</small>}</td><td><span className={`adm-status-pill ${row.state === 'done' ? 'is-active' : row.state === 'stalled' || row.state === 'needs_retry' ? 'is-warning' : 'is-archived'}`}>{EFFORT[row.state] || row.state}</span></td><td>{row.sections_total ? `${row.sections_done}/${row.sections_total} phần` : `${row.stages_done}${effort.stages_total ? `/${effort.stages_total}` : ''} chặng`}</td><td>{row.attempts}</td><td>{row.combined_pct == null ? '—' : `${Math.round(row.combined_pct)}%`}</td><td>{row.attempt_minutes ? `${row.attempt_minutes}′` : row.minutes == null ? '—' : `${row.minutes}′`}</td><td><button className="adm-btn-secondary adm-btn-sm" type="button" disabled={!row.user_id} onClick={() => void openStudent({ student_id: row.student_id, user_id: row.user_id, name, has_writing: tallyRow?.has_writing || row.wrote })}>Xem từng lượt</button></td></tr>; })}</tbody></table></div><p className="acd-muted">Thời gian là tổng thời gian mỗi phần được mở trong lượt làm; không suy từ khoảng cách giữa hai timestamp server.</p></>}{effort.axes.length > 0 && <section className="acs-class-axes"><h3>Cả lớp vướng ở đâu</h3>{effort.axes.map((axis) => <div key={axis.axis}><span>{axis.axis}</span><strong>{axis.wrong} sai</strong><em>{axis.median_sec == null ? '—' : `${axis.median_sec}s trung vị`}</em></div>)}</section>}</div>}
