...
function evidenceText(value: unknown) {
  if (value == null) return 'Không trả lời';
  if (typeof value === 'string') return value || 'Không trả lời';
...
function AdvancedVocabularyReport({ data }: { data: AdvancedVocabularyStudentResult }) {
  ...
  const completed = new Set([
    ...data.stages.filter((row) => row.status === 'completed').map((row) => row.stage),
    ...data.sections.map((row) => row.section),
  ]);
  const learnerState = advancedVocabularyStudentState(data);
  const stateLabel = learnerState === 'done' ? 'Đã hoàn tất đủ phần bắt buộc' : learnerState === 'doing' ? 'Đang hoàn thành' : 'Chưa mở bài';
  const initialListening = data.sections.some((row) => row.section === 'listening') ? [] : data.listening_attempts;
  return <div className="acs-report">
    <div className="acd-warning"><strong>Bài self-paced không có điểm tổng.</strong> Admin vẫn xem được tiến độ, kết quả từng câu và thời gian từng phần. Writing và Speaking chỉ là nội dung tham khảo; bài Writing được chấm vẫn phải do giáo viên giao riêng.</div>
    <section className="acs-decision" aria-labelledby="acs-advanced-decision"><div><p className="acd-eyebrow">Trạng thái bài học</p><h4 id="acs-advanced-decision">{stateLabel}</h4><span>{completed.size}/{total} phần · {data.practice_attempts.length} lượt tương tác luyện tập</span></div><div className="acs-decision-score"><strong>{completed.size}/{total}</strong><span>Phần hoàn tất</span></div></section>
    <section className="acs-section-grid" aria-label="Tiến độ từng phần">{requiredStages.map((key) => [key, labels[key] || key] as const).map(([key, label]) => {
      const section = data.sections.find((row) => row.section === key);
      return <article key={key}><span>{label}</span><strong>{completed.has(key) ? 'Hoàn tất' : 'Chưa xong'}</strong>{section && <em>{section.correct}/{section.total} câu · {Math.round(section.duration_sec / 60)} phút</em>}</article>;
    })}</section>
    <section className="acs-history"><div><p className="acd-eyebrow">Bằng chứng đã lưu</p><h4>Reading và Listening</h4><span>Kết quả của hai phần bắt buộc được giữ riêng, không gộp thành điểm khóa học.</span></div><div className="acd-table-scroll" tabIndex={0}><table className="acd-table"><thead><tr><th>Phần</th><th>Kết quả</th><th>Thời lượng</th><th>Nộp lúc</th></tr></thead><tbody>{data.sections.map((row) => <tr key={row.section}><td>{labels[row.section] || row.section}</td><td>{row.correct}/{row.total} câu</td><td>{Math.round(row.duration_sec / 60)} phút</td><td>{formatVietnam(row.submitted_at)}</td></tr>)}{initialListening.map((row, index) => <tr key={`listening-initial-${index}`}><td>Listening · lượt đầu (đang sửa)</td><td>{row.correct}/{row.total} câu<small>{Object.keys(row.answers).length} câu trả lời đã lưu</small></td><td>{Math.round(row.duration_sec / 60)} phút</td><td>{formatVietnam(row.submitted_at)}</td></tr>)}{!data.sections.length && !initialListening.length && <tr><td colSpan={4}>Chưa hoàn tất Reading hoặc Listening.</td></tr>}</tbody></table></div></section>
...
export function AdminClassSubmissions({ cohortId, assignment, initialStudent = null, backLabel = 'Quay lại danh sách bài tập', memberNames, memberUserIds, memberNamesAvailable, onBack, onMutation }: SubmissionWorkspaceProps) {
  ...
  const [advancedReport, setAdvancedReport] = useState<AdvancedVocabularyStudentResult | null>(null);
  ...
  useEffect(() => {
    setView('tally'); setTally(null); setEffort(null); setReport(null); setAdvancedReport(null); setWriting(null); setSelected(null); setBanner(null);
    void (async () => {
  ...
  async function openStudent(row: { student_id?: string | null; studentId?: string | null; user_id?: string | null; userId?: string | null; name: string; has_writing?: boolean; hasWriting?: boolean }) {
    ...
    setView('student'); setSelected({ studentId, userId: initialUserId, name: row.name || 'Học viên', hasWriting }); setReport(null); setAdvancedReport(null); setWriting(null); setError(null); setLoading(true);
  ...
  const returnWork = async () => {
    ...
    try {
      ...
      const canonical = await loadTally(true); await onMutation(); setView('tally'); setSelected(null); setWriting(null); setReport(null); setAdvancedReport(null); setEffort(null);
  ...
  return <section id="acx-panel-homework" className="acd-workspace acs-workspace" role="tabpanel" aria-labelledby="acx-tab-homework">
    ...
    {!loading && !error && view === 'tally' && tally && <div id="acs-panel-tally" role="tabpanel" aria-labelledby="acs-tab-tally" className="acs-tally">
      {bankId ? <section className="acs-outcomes" aria-label="Tổng kết kết quả học tập">
        <article data-kind="passed"><strong>{tally.counts.passed}</strong><span>{isAdvancedVocabulary ? 'Đã hoàn tất' : 'Đã đạt'}</span></article>
        {!isAdvancedVocabulary && <article data-kind="near"><strong>{tally.counts.near_pass}</strong><span>Gần đạt · Revision</span></article>}
        {!isAdvancedVocabulary && <article data-kind="retry"><strong>{tally.counts.retry_full}</strong><span>Làm lại toàn bài</span></article>}
