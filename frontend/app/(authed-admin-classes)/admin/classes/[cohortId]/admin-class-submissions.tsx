...
function AdvancedVocabularyReport({ data }: { data: AdvancedVocabularyStudentResult }) {
  ...
  return <div className="acs-report">
    ...
    <section className="acs-decision" aria-labelledby="acs-advanced-decision"><div><p className="acd-eyebrow">Trạng thái bài học</p><h4 id="acs-advanced-decision">{data.item.submitted_at ? 'Đã hoàn tất đủ phần bắt buộc' : 'Đang hoàn thành'}</h4><span>{completed.size}/{total} phần · {data.practice_attempts.length} lượt tương tác luyện tập</span></div><div className="acs-decision-score"><strong>{completed.size}/{total}</strong><span>Phần hoàn tất</span></div></section>
    ...
    <section className="acs-history"><div><p className="acd-eyebrow">Bằng chứng đã lưu</p><h4>Reading và Listening</h4><span>Kết quả của hai phần bắt buộc được giữ riêng, không gộp thành điểm khóa học.</span></div><div className="acd-table-scroll" tabIndex={0}><table className="acd-table"><thead><tr><th>Phần</th><th>Kết quả</th><th>Thời lượng</th><th>Nộp lúc</th></tr></thead><tbody>{data.sections.length ? data.sections.map((row) => <tr key={row.section}><td>{labels[row.section] || row.section}</td><td>{row.correct}/{row.total} câu</td><td>{Math.round(row.duration_sec / 60)} phút</td><td>{formatVietnam(row.submitted_at)}</td></tr>) : <tr><td colSpan={4}>Chưa hoàn tất Reading hoặc Listening.</td></tr>}</tbody></table></div></section>
