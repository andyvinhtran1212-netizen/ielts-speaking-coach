...
export function ProgrammeResult({ attemptId }: { attemptId: string }) {
  ...
  return <main className="programme-result">
    ...
    <header className="programme-result__hero"><p>{result.assisted ? 'Hoàn thành có hỗ trợ' : 'Tự đối chiếu sau bài luyện'}</p><h1>{result.title}</h1><div><a href={programmeLibraryPath}>← Thư viện chương trình</a><span>Hoàn thành {summary.completion_count || 0}/{summary.item_count || result.review.length} câu</span></div></header>
