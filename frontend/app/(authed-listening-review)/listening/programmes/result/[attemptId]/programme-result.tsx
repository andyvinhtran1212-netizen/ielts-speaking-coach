...
interface ReviewItem { qNum: number; state: string; correct: boolean | null; userAnswer: string; expected: string | string[]; prompt: string; responseType: string; audioWindow?: { start?: number; end?: number }; solution: Record<string, unknown>; selfReview: Record<string, unknown> }
interface Result { title: string; programmeId: string; lessonId: string; audioUrl: string; summary: Record<string, number>; review: ReviewItem[]; transcripts: Record<string, Array<Record<string, unknown>>> }
...
export function ProgrammeResult({ attemptId }: { attemptId: string }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const review = (Array.isArray(payload.review) ? payload.review : []).map((value) => { const item = row(value); return { qNum: Number(item.q_num), state: String(item.state || (item.correct == null ? 'unscored' : 'checked')), correct: typeof item.correct === 'boolean' ? item.correct : null, userAnswer: String(item.user_answer || ''), expected: item.expected as string | string[], prompt: String(item.prompt || ''), responseType: String(item.question_type || ''), audioWindow: row(item.audio_window) as ReviewItem['audioWindow'], solution: row(item.solution), selfReview: row(item.self_review) }; });
      ...
      if (active) setState({ status: 'ready', result: { title: String(payload.title || 'Kết quả luyện nghe'), programmeId: String(payload.programme_id || ''), lessonId: String(payload.listening_lesson_id || ''), audioUrl: String(payload.audio_url || ''), summary, review, transcripts: row(payload.controlled_transcripts) as Result['transcripts'] } });
  ...
  return <main className="programme-result">
    <audio ref={audio} src={result.audioUrl} preload="metadata" />
    <header className="programme-result__hero"><p>Tự đối chiếu sau bài luyện</p><h1>{result.title}</h1><div><a href={result.lessonId ? `/listening/${lessonProgrammePath}/${result.lessonId}` : '/listening'}>← Quay lại bài học</a><span>Hoàn thành {summary.completion_count || 0}/{summary.item_count || result.review.length} câu</span></div></header>
    <section className="programme-result__notice"><strong>Đây không phải điểm IELTS.</strong><span>Câu lựa chọn/bản đồ được kiểm tra chính xác; câu viết được giữ nguyên để bạn tự so với gợi ý.</span></section>
    <section className="programme-result__summary" aria-label="Tóm tắt kết quả"><div><strong>{summary.correct_count || 0}/{summary.checked_count || 0}</strong><span>Câu được kiểm tra đúng</span></div><div><strong>{summary.unscored_count || 0}</strong><span>Câu tự đối chiếu</span></div><div><strong>{summary.blank_count || 0}</strong><span>Câu để trống</span></div>{summary.technical_error_count ? <div className="is-error"><strong>{summary.technical_error_count}</strong><span>Lỗi kỹ thuật</span></div> : null}</section>
    <section className="programme-result__items"><h2>Đối chiếu từng câu</h2>{result.review.map((item) => { const references = list(item.selfReview.reference_answers); const facts = list(item.selfReview.required_facts); const expected = Array.isArray(item.expected) ? item.expected : item.expected ? [String(item.expected)] : list(item.solution.expected); return <article className="programme-review-item" data-state={item.state} key={item.qNum}><header><span>{item.qNum}</span><div><h3>{item.prompt}</h3><p>{item.state === 'checked' ? item.correct ? 'Đúng' : 'Chưa đúng' : item.state === 'unscored' ? 'Tự đối chiếu' : item.state === 'blank' ? 'Để trống' : 'Cần thử lại sau'}</p></div>{item.audioWindow?.start != null ? <button type="button" onClick={() => replay(item)}>▶ Nghe đoạn liên quan</button> : null}</header><div className="programme-review-item__answer"><span>Câu trả lời của bạn</span><p>{item.userAnswer || '—'}</p></div>{expected.length ? <div className="programme-review-item__reference"><span>Đáp án đối chiếu</span><p>{expected.join(' · ')}</p>{item.solution.rationale ? <small>{String(item.solution.rationale)}</small> : null}</div> : null}{item.state === 'unscored' ? <div className="programme-review-item__reference"><span>Gợi ý tự kiểm tra</span>{references.length ? <p>{references.join(' · ')}</p> : null}{facts.length ? <ul>{facts.map((fact) => <li key={fact}>{fact}</li>)}</ul> : null}{item.selfReview.rationale ? <small>{String(item.selfReview.rationale)}</small> : null}</div> : null}</article>; })}</section>
