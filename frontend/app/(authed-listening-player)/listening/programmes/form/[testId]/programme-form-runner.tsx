  ...
  return <main className="programme-runner shell">
    ...
    <div className="programme-learning-workspace">
      ...
      {visibleQuestions.map((sourceQuestion) => { const question = languages.length ? displayQuestion(sourceQuestion, activeLanguage) as Question : sourceQuestion; const item = feedback[question.q_num]; const revealing = revealStatus[question.q_num] === 'revealing'; return <article className="programme-question" key={question.q_num}>
        ...
        {question.visual_url ? <img src={question.visual_url} alt={question.visual_accessibility || 'Sơ đồ cho câu hỏi'} lang={languages.length ? activeLanguage : undefined} /> : null}
        {['single_choice', 'map_label'].includes(question.response_type) ? <div className="programme-options">{Object.entries(question.options).map(([key, label]) => <label key={key}><input type="radio" name={`q-${question.q_num}`} checked={answers[question.q_num] === key} disabled={submitting || revealing} onChange={() => update(question.q_num, key, true)} /><span lang={languages.length ? activeLanguage : undefined}><strong>{key}</strong>{label}</span></label>)}</div> : null}
        {question.response_type === 'multiple_choice' ? <div className="programme-options">{Object.entries(question.options).map(([key, label]) => <label key={key}><input type="checkbox" checked={(answers[question.q_num] || '').split(',').map((value) => value.trim()).includes(key)} disabled={submitting || revealing} onChange={() => toggleMultiple(question, key)} /><span lang={languages.length ? activeLanguage : undefined}><strong>{key}</strong>{label}</span></label>)}</div> : null}
