...
import {
  ...
  MAX_LISTENING_SEGMENTS, normalizeListeningDictationBlocks,
...
export function AdminListeningSegments({ contentId, requestedExerciseId }: { contentId: string; requestedExerciseId: string | null }) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (nextCollection.malformedCount) throw new Error(`Có ${nextCollection.malformedCount} Dictation row sai contract. Editor khóa để không tạo block đè lên order đang tồn tại; hãy sửa dữ liệu nguồn rồi thử lại.`);
  ...
  return <main className="alse-shell">
    ...
    <section className="alse-workspace">
      ...
      <aside className="alse-side"><section><p className="alc-eyebrow">Block identity</p><h2>{collection.items.length ? `${collection.items.length} Dictation block` : 'Chưa có block'}</h2>{collection.items.length > 0 && <label className="alse-field" htmlFor="alse-block"><span>Block đang sửa</span><select id="alse-block" value={selectedId || ''} disabled={locked} onChange={(event) => chooseBlock(event.target.value)}>{!baseline && <option value="">Block mới #{orderNum}</option>}{collection.items.map((item) => <option key={item.id} value={item.id}>#{item.orderNum} · {STATUS_LABEL[item.status]} · {item.segments.length} câu</option>)}</select></label>}<button className="adm-btn-secondary" type="button" disabled={locked || Math.max(0, ...collection.items.map((item) => item.orderNum)) >= 200} onClick={() => dirty ? setConfirm('new-block') : startNewBlock()}>Thêm Dictation block</button><dl><div><dt>Exercise ID</dt><dd><code>{baseline?.id || 'Tạo mới khi lưu'}</code></dd></div><div><dt>Order</dt><dd><code>#{orderNum}</code></dd></div><div><dt>Version</dt><dd><code>{baseline?.updatedAt || 'expected_absent'}</code></dd></div><div><dt>Nguồn</dt><dd>{content.sourceType || '—'}</dd></div></dl></section><section><p className="alc-eyebrow">Publication</p><fieldset className="alse-status-options"><legend>Trạng thái đích</legend><label className="alse-radio"><input type="radio" name="status" checked={targetStatus === 'draft'} disabled={locked} onChange={() => setTargetStatus('draft')} /><span><strong>Bản nháp</strong><small>Chưa phục vụ người học.</small></span></label><label className="alse-radio"><input type="radio" name="status" checked={targetStatus === 'published'} disabled={locked} onChange={() => setTargetStatus('published')} /><span><strong>Xuất bản</strong><small>Chỉ khả dụng khi content cha cũng published.</small></span></label><label className="alse-radio"><input type="radio" name="status" checked={targetStatus === 'archived'} disabled={locked} onChange={() => setTargetStatus('archived')} /><span><strong>Lưu trữ</strong><small>Giữ block nhưng không phục vụ người học.</small></span></label></fieldset></section></aside>
    </section>
