...
import {
  ...
  listeningGistDraft, listeningGistHref,
...
export function AdminListeningGist({ contentId, requestedExerciseId }: { contentId: string; requestedExerciseId: string | null }) {
  ...
  if (loadError || !content || !collection) return <main className="alge-shell"><div className="alc-banner is-error" role="alert"><strong>Không mở được trình biên tập Gist</strong><span>{loadError || 'Không có canonical snapshot hợp lệ.'}</span></div><div className="alge-state-actions"><a className="adm-btn-secondary" href="/admin/listening">Về Listening</a><button className="adm-btn-primary" type="button" onClick={() => void load()}>Thử lại</button></div></main>;
