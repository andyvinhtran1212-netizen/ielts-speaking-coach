...
export function AdminListeningTrueFalse({ contentId, requestedExerciseId }: { contentId: string; requestedExerciseId: string | null }) {
  ...
  const focusOrder = useRef<number | null>(null);
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const requestedOrder = focusOrder.current;
      const block = requestedId
        ? nextCollection.items.find((item) => item.id === requestedId) || null
        : requestedOrder != null
          ? nextCollection.items.find((item) => item.orderNum === requestedOrder) || null
      ...
      if (requestedOrder != null && !block) throw new Error(`Không tìm thấy T/F block order ${requestedOrder} vừa xung đột.`);
  ...
  const save = async (status: Status) => {
    ...
    try {
      ...
      if (!postAcknowledged && definitive(statusCode)) {
        ...
        if (statusCode === 409) {
          if (!baseline) focusOrder.current = orderNum;
          setConflict(true);
        }
  ...
  if (loadError || !content || !collection) return <main className="altf-shell"><div className="alc-banner is-error" role="alert"><strong>Không mở được trình biên tập True / False</strong><span>{loadError || 'Không có canonical snapshot hợp lệ.'}</span></div><div className="altf-state-actions"><a className="adm-btn-secondary" href="/admin/listening">Về Listening</a>{contentId && <a className="adm-btn-secondary" href={listeningTrueFalseRollbackHref(contentId, requestedExerciseId)}>Mở HTML rollback</a>}<button className="adm-btn-primary" type="button" onClick={() => void load()}>Thử lại</button></div></main>;
