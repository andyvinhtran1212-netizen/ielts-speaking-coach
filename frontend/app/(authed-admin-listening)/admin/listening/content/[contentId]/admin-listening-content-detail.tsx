...
function ExerciseMatrix({ state, contentId }: { state: ReadState<Coverage>; contentId: string }) {
  ...
  return <div className="ald-exercise-grid">{state.value.items.map((item) => { const editor = item.type === 'dictation' ? `/admin/listening/segments?content_id=${encodeURIComponent(contentId)}` : `/pages/admin/listening/${item.type === 'true_false' ? 'tf' : item.type}.html?content_id=${encodeURIComponent(contentId)}`; return <article key={item.type}><div><strong>{LISTENING_EXERCISE_LABEL[item.type]}</strong><span className={`adm-status-pill ${statusClass(item.status)}`}>{item.status || 'Chưa có'}</span></div><p>{item.blockCount} block · {item.status === 'mixed' ? 'nhiều trạng thái' : item.status || 'chưa soạn'}</p><a className="adm-btn-secondary" href={editor}>{item.blockCount ? 'Mở editor' : 'Tạo bài'} ↗</a></article>; })}<p className="ald-exercise-summary">{state.value.presentCount}/4 loại · {state.value.blockCount} block · {state.value.readyCount}/4 published{state.value.supplementalCount ? ` · ${state.value.supplementalCount} mini-test` : ''}{state.value.malformedCount ? ` · ${state.value.malformedCount} dòng sai contract` : ''}</p></div>;
}
