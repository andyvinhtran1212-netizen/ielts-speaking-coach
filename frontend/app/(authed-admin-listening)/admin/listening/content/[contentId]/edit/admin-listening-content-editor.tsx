...
export function AdminListeningContentEditor({ contentId }: { contentId: string }) {
  ...
  return <main className="alme-shell">
    ...
    <form className="alme-layout" onSubmit={(event) => void save(event)} noValidate>
      ...
      <aside className="alme-side"><section className="alme-side-card"><p className="alc-eyebrow">Read-only truth</p><h2>Nguồn kỹ thuật</h2><dl><div><dt>Source type</dt><dd>{baseline.sourceType || '—'}</dd></div><div><dt>Audio</dt><dd>{baseline.audioStoragePath ? 'Có storage path' : 'Chưa có'}</dd></div><div><dt>Trạng thái</dt><dd>{STATUS_COPY[baseline.status]}</dd></div><div><dt>Version</dt><dd><code>{baseline.updatedAt}</code></dd></div></dl><p>Audio, duration, storage path và alignment không sửa ở màn này.</p></section><section className="alme-side-card"><p className="alc-eyebrow">Delta preview</p><h2>{changed.length ? `${changed.length} trường sẽ PATCH` : dirty ? 'Dữ liệu chưa hợp lệ' : 'Không có delta'}</h2><ul>{changed.map((key) => <li key={key}>{fieldNames[key] || key}</li>)}</ul></section></aside>
