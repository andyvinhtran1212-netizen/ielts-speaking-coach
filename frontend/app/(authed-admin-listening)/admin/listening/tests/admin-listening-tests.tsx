...
type Row = { id: string; testId: string; status: Exclude<Status, 'all'>; type: Exclude<TestType, 'all' | 'exam'>; title: string; bandTarget: number | null; accentProfile: string[]; sectionCount: number; audioReadyCount: number; examOnly: boolean; isPublic: boolean; createdAt: string | null; updatedAt: string | null };
type Snapshot = { key: string; rows: Row[]; total: number; malformedCount: number; readAt: string };
type Action = { row: Row; kind: 'status'; status: Exclude<Status, 'all'> } | { row: Row; kind: 'visibility'; isPublic: boolean };
...
export function AdminListeningTests() {
  ...
  const mutate = async () => {
    ...
    try {
      ...
      else await window.api.patch(`/admin/listening/tests/${encodeURIComponent(target.row.id)}`, { is_public: target.isPublic });
      const readback = normalizeListeningTestMutationReadback(await window.api.get<unknown>(`/admin/listening/tests/${encodeURIComponent(target.row.id)}`), target.row.id, target.kind === 'status' ? { status: target.status } : { isPublic: target.isPublic });
      if (!readback) throw new Error('Backend chưa xác nhận thay đổi vừa chọn.');
      if (scope.current !== owner) return;
      const refreshed = await load(false);
      if (scope.current !== owner) return;
      setAction(null);
      const saved = target.kind === 'status' ? LISTENING_STATUS_LABEL[target.status] : target.isPublic ? 'công khai trên web' : 'ẩn khỏi kho tự luyện';
  ...
  return <main className="alt-shell">
    <header className="alt-hero"><div><p className="alc-eyebrow">Listening · Test operations</p><h1>Kho test Listening</h1><p>Đối chiếu cấu trúc section, audio và chọn đề công khai hoặc ẩn độc lập với mock test.</p></div><div className="alt-hero__actions"><a className="adm-btn-primary" href="/admin/listening/import-fulltest">Import full test</a><a className="adm-btn-secondary" href="/admin/listening/import-drills">Import skill drill</a><a className="adm-btn-secondary" href="/admin/listening/audit">Audit chất lượng</a></div></header>
    <section className="alt-boundary" aria-labelledby="alt-boundary-title"><div><p className="alc-eyebrow">Visibility boundary</p><h2 id="alt-boundary-title">Published và hiển thị web là hai trạng thái riêng</h2></div><div><span className="adm-status-pill is-live">Published</span><p>Đã qua cổng audio và có thể được phân phối.</p></div><div><span className="alt-scope is-exam">Đang ẩn</span><p>Vẫn có thể gán mock test hoặc giao riêng cho lớp.</p></div></section>
