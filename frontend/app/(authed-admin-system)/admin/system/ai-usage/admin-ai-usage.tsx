  ...
  return (
    <main className="aiun-shell">
      ...
      {payload && <>
        ...
        {payload.meta.ledgerSchemaLegacy && <div className="aiun-banner is-warning" role="status">Ledger đang chạy schema tương thích cũ; cần áp dụng migration 284 để có đầy đủ trạng thái, feature và chống trùng.</div>}
        {payload.meta.writingSourceTruncated && <div className="aiun-banner is-warning" role="status">Nguồn Writing lịch sử đã chạm giới hạn {formatCount(payload.meta.writingSourceReturnedRows)} / {formatCount(payload.meta.writingSourceTotalRows)} dòng; tổng chi phí có thể chưa đầy đủ.</div>}
        {payload.meta.writingLookupFailed && <div className="aiun-banner is-warning" role="status">Không đọc được log Writing lịch sử; tổng chi phí hiện tại có thể thiếu các lượt chấm trước khi ledger mới được bật.</div>}
        {(payload.meta.supplementalWritingRows ?? 0) > 0 && <div className="aiun-banner" role="status">Đã bổ sung {formatCount(payload.meta.supplementalWritingRows)} lượt chấm Writing cũ từ dữ liệu phản hồi; không thay đổi dữ liệu gốc.</div>}
        {(payload.overall.unpricedCalls ?? 0) > 0 && <div className="aiun-banner is-warning" role="status">{formatCount(payload.overall.unpricedCalls)} lượt gọi chưa có đơn giá USD trong catalog; tổng tiền bên dưới chưa bao gồm các lượt này.</div>}
        {(payload.overall.legacyRepricedCalls ?? 0) > 0 && <div className="aiun-banner" role="status">{formatCount(payload.overall.legacyRepricedCalls)} log cũ đã được tính lại bằng bảng giá theo model; dữ liệu gốc không bị sửa.</div>}
