...
export function ListeningMiniTestBehavior() {
  ...
  if (status !== 'signed-in' || !user?.id) {
    return <div className="empty-state" id="state-loading">Đang tải danh sách mini tests…</div>;
  }

  return <ListeningMiniTestLibrary accountKey={user.id} key={user.id} />;
}

function ListeningMiniTestLibrary({ accountKey }: { accountKey: string }) {
