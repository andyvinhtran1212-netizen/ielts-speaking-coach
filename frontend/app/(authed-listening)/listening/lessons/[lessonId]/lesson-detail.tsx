...
interface FormCard { id: string; title: string; purpose: string; replay: string; support: string; itemCount: number; durationSeconds: number; checkedCount: number; selfReviewCount: number; status: string; attemptId: string | null }
...
export function ListeningLessonDetail({ lessonId }: { lessonId: string }) {
  ...
  return <main className="shell listening-lesson-detail">
    ...
    {state.status === 'ready' ? <>
      ...
      <section className="listening-form-list" aria-labelledby="form-list-title"><div className="listening-section-heading"><div><p className="listening-kicker">Các lượt luyện</p><h2 id="form-list-title">Chọn bài nghe</h2></div><span>Không chấm band · có tự đối chiếu sau khi nộp</span></div>
        {state.lesson.forms.length ? state.lesson.forms.map((form, index) => {
          ...
          return <article className="listening-form-card" key={form.id}><span className="listening-form-card__index">{String(index + 1).padStart(2, '0')}</span><div><p>{PURPOSE[form.purpose] || 'Bài nghe'} · {form.itemCount} câu · {duration}</p><h3>{form.title}</h3><small>{form.replay === 'once' ? 'Nghe một lần' : 'Được nghe lại'} · {support}{feedback ? ` · ${feedback}` : ''}</small></div><a href={resultHref || `/listening/programmes/form/${form.id}`}>{resultHref ? 'Xem lại' : form.status === 'in_progress' ? 'Tiếp tục' : 'Bắt đầu'} →</a></article>;
