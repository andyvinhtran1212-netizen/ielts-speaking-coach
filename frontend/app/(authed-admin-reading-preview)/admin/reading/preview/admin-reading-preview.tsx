  ...
  return <main className="arp-shell">
    ...
    {!current ? <div className="arp-state" role="status"><span className="arp-spinner"/><strong>{loading ? 'Đang đọc đề và answer key…' : 'Chưa có snapshot canonical'}</strong></div> : <>
      ...
      {!test!.passages.length ? <div className="arp-state is-error"><strong>Không có passage hợp lệ</strong><p>Không thể kiểm định câu hỏi cho tới khi payload được sửa.</p></div> : <div className="arp-workspace">
        <aside className="arp-nav" aria-label="Điều hướng passage"><div className="arp-nav__head"><span>Cấu trúc đề</span><button type="button" onClick={() => void load()} disabled={loading || Boolean(busyQuestion)}>{loading ? 'Đang tải…' : 'Làm mới'}</button></div>{test!.passages.map((item) => { const count = questionsByPassage(test, item.id).length; return <button type="button" className={item.id === passage?.id ? 'is-active' : ''} aria-current={item.id === passage?.id ? 'true' : undefined} key={item.id} onClick={() => { setActivePassage(item.id); window.scrollTo({ top: 0, behavior: window.matchMedia('(prefers-reduced-motion: reduce)').matches ? 'auto' : 'smooth' }); }}><span>Passage {item.order}</span><strong>{item.title}</strong><small>{count} câu · {item.wordCount ?? '—'} từ</small></button>; })}<a href={`/pages/reading-review.html?admin_test_id=${encodeURIComponent(testId)}`} target="_blank" rel="noreferrer">Xem như học viên ↗</a></aside>
