...
function Icon({ name, className = '' }: { name: string; className?: string }) {
  ...
  switch (name) {
    case 'chevron-left':
      ...
      break;
    case 'alert-triangle':
      paths = <><path d="m21.73 18-8-14a2 2 0 0 0-3.46 0l-8 14A2 2 0 0 0 4 21h16a2 2 0 0 0 1.73-3" /><path d="M12 9v4" /><path d="M12 17h.01" /></>;
      break;
    case 'eye':
      paths = <><path d="M2.06 12.35a1 1 0 0 1 0-.7C3.53 7.48 7.47 5 12 5c4.53 0 8.47 2.48 9.94 6.65a1 1 0 0 1 0 .7C20.47 16.52 16.53 19 12 19c-4.53 0-8.47-2.48-9.94-6.65" /><circle cx="12" cy="12" r="3" /></>;
      break;
    case 'volume-2':
      paths = <><path d="M11 5 6 9H2v6h4l5 4z" /><path d="M15.54 8.46a5 5 0 0 1 0 7.07" /><path d="M19.07 4.93a10 10 0 0 1 0 14.14" /></>;
      break;
    case 'mic':
      paths = <><path d="M12 2a3 3 0 0 0-3 3v7a3 3 0 0 0 6 0V5a3 3 0 0 0-3-3" /><path d="M19 10v2a7 7 0 0 1-14 0v-2" /><line x1="12" x2="12" y1="19" y2="22" /></>;
      break;
    case 'square':
      paths = <rect width="14" height="14" x="5" y="5" rx="2" />;
      break;
    case 'rotate-ccw':
      paths = <><path d="M3 12a9 9 0 1 0 3-6.7L3 8" /><path d="M3 3v5h5" /></>;
      break;
    case 'check-circle':
    case 'check-circle-2':
      paths = <><path d="M22 11.08V12a10 10 0 1 1-5.93-9.14" /><path d="m9 11 3 3L22 4" /></>;
      break;
    case 'upload':
      paths = <><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4" /><polyline points="17 8 12 3 7 8" /><line x1="12" x2="12" y1="3" y2="15" /></>;
      break;
    case 'play':
    ...
    case 'download':
      ...
      break;
    case 'file-down':
      paths = <><path d="M14.5 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V7.5z" /><polyline points="14 2 14 8 20 8" /><path d="M12 18v-6" /><path d="m9 15 3 3 3-3" /></>;
      break;
    default:
      paths = null;
  }
    ...
    <>
      ...
      <main className="flex-1 flex flex-col">
        ...
        <div id="state-feedback" className={stateClass('feedback', 'state block-state flex-1 av-w-read py-8 ds-fadein')}>
          ...
          <div className="card p-5 mb-4"><p className="practice-card-eyebrow text-xs font-bold uppercase tracking-wider mb-3">Nhận xét</p><div id="feedback-comments" /></div>
          <div id="feedback-transcript" className="card p-4 mb-4" style={{ display: 'none' }}><p className="practice-card-eyebrow practice-card-eyebrow--faint text-xs font-bold uppercase tracking-wider mb-2">Văn bản nhận dạng</p><p id="feedback-transcript-text" className="practice-transcript-text text-xs leading-relaxed" /></div>
          <div id="feedback-audio-section" className="practice-feedback-audio" style={{ display: 'none' }}><div className="flex gap-2"><button type="button" className="btn-ghost practice-icon-btn py-2.5 text-xs font-semibold" style={{ flex: 1 }} onClick={() => callPractice('replayAudio')}><Icon name="volume-2" />Nghe lại</button><button type="button" className="btn-ghost practice-icon-btn py-2.5 text-xs font-semibold" style={{ flex: 1 }} onClick={() => callPractice('downloadAudio')}><Icon name="download" />Tải audio</button></div></div>
