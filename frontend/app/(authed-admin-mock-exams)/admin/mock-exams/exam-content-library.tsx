...
import { getAdminExamContent } from '@/lib/admin-exam-content-api';
...
export function ExamContentLibrary({ accountId, cohorts }: Props) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const normalized = normalizeExamContent(await getAdminExamContent(query));
  ...
  const tabLevels = useMemo(() => [...new Set([...levels, ''])].sort((a, b) => !a ? 1 : !b ? -1 : a.localeCompare(b)), [levels]);
  ...
  return (
    <section className="mex-card mex-content" id="test-library" role="tabpanel" aria-labelledby="mex-library-tab">
      ...
      <div className="mex-toolbar">
        <label className="mex-search"><span>Tìm đề</span><input type="search" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Mã, tiêu đề hoặc cấp khóa…" /></label>
        ...
        <label><span>Cấp khóa</span><select value={levelTab === null ? '__all__' : levelTab} onChange={(event) => setLevelTab(event.target.value === '__all__' ? null : event.target.value)}><option value="__all__">Tất cả</option><option value="">Chưa đặt</option>{levels.map((level) => <option key={level} value={level}>{level}</option>)}</select></label>
      ...
      <div className="mex-level-tabs" role="tablist" aria-label="Lọc nhanh theo cấp khóa">
        <button type="button" className={levelTab === null ? 'is-active' : ''} onClick={() => setLevelTab(null)}>Tất cả <small>{total}</small></button>
      ...
      <div className="mex-table-wrap">
        {loading && !rows.length ? <p role="status">Đang tải kho đề…</p> : !visible.length ? <p>Không có đề khớp bộ lọc.</p> : <table className="mex-table"><thead><tr><th>Kỹ năng</th><th>Mã / tiêu đề</th><th>Sẵn sàng</th><th>Hiển thị web</th><th>Mock test</th><th>Cấp khóa</th><th>Lớp</th><th>Thao tác</th></tr></thead><tbody>{pageRows.map((row) => {
      ...
      {total > 0 && <div className="mex-pagination" aria-label="Phân trang kho đề"><span>Hiển thị {(currentPage - 1) * pageSize + 1}–{Math.min(currentPage * pageSize, total)} / {total} đề</span><label><span>Số dòng</span><select value={pageSize} onChange={(event) => { setPageSize(Number(event.target.value)); setPage(1); }}><option value={25}>25</option><option value={50}>50</option></select></label><div><button className="adm-btn-secondary" type="button" onClick={() => setPage((value) => Math.max(1, value - 1))} disabled={currentPage === 1}>Trang trước</button><span>Trang {currentPage}/{pageCount}</span><button className="adm-btn-secondary" type="button" onClick={() => setPage((value) => Math.min(pageCount, value + 1))} disabled={currentPage === pageCount}>Trang sau</button></div></div>}
