...
import {
  ...
  normalizeDeleteAck, normalizeLockAck, normalizeReadingContentList,
  normalizeReadingVisibilityAck,
...
type Row = { id: string; slug: string; library: Exclude<Library, ''>; title: string; status: string; difficultyLevel: string | null; skillFocus: string | null; topicTags: string[]; examOnly: boolean; isPublic: boolean; locked: boolean; shareActive: boolean; shareExpiresAt: string | null; contentAuditStatus: string | null; contentAuditedAt: string | null; updatedAt: string | null; createdAt: string | null };
  ...
  const runAction = async () => {
    ...
    try {
      if (action.kind === 'exam') {
        ...
        if (!normalizeReadingVisibilityAck(await window.api.patch<unknown>(`/admin/reading/content/tests/${encodeURIComponent(row.slug)}/visibility`, { is_public: next }), row.slug, next)) throw new Error('ACK visibility không khớp.');
        const canonical = await findCanonical('l3_test', row.slug);
        if (!canonical || canonical.isPublic !== next) throw new Error('Canonical readback chưa phản ánh visibility.');
        setBanner({ kind: 'success', text: next ? 'Đã mở đề công khai trên web.' : 'Đã ẩn đề khỏi kho tự luyện.' });
