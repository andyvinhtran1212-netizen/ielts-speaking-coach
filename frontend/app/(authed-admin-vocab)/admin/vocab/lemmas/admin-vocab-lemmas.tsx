...
export function AdminVocabLemmas() {
  ...
  useEffect(() => {
    ...
    const onKeyDown = (event: KeyboardEvent) => {
      ...
      setCreateOpen(false); setDeleteRow(null);
    };
  ...
  const createOverride = async () => {
    ...
    if (!body.original_word || !body.lemma) {
      setNotice({ kind: 'error', message: 'Original word và lemma không được trống.' });
      return;
    }
    if (!LEMMA_POS_TAGS.includes(draft.posTag)) {
      setNotice({ kind: 'error', message: 'POS tag không hợp lệ.' });
      return;
    }
    ...
    try {
      ...
      if (account === accountRef.current) setNotice({ kind: 'error', message: `Không xác nhận được trạng thái override: ${messageOf(caught)}` });
    } finally {
  ...
  const removeOverride = async () => {
    ...
    try {
      ...
      if (account === accountRef.current) setNotice({ kind: 'error', message: `Không xác nhận được trạng thái xoá: ${messageOf(caught)}` });
    } finally {
      mutationLock.current = false;
      if (account === accountRef.current) setBusy(false);
    }
  ...
  return (
    <main className="avv-shell avv-console-shell">
      ...
      <form className="avv-searchbar" onSubmit={(event) => { event.preventDefault(); applySearch(); }}>
        <label><span className="sr-only">Tìm original word</span><input aria-label="Tìm original word" value={draftSearch} onChange={(event) => setDraftSearch(event.target.value)} placeholder="Tìm theo original word (prefix match)…" /></label>
        <button className="btn-secondary" type="submit">Tìm</button>
        <button className="btn-primary" type="button" onClick={() => { setDraft(EMPTY_DRAFT); setNotice(null); setCreateOpen(true); }}>+ Thêm override</button>
      </form>
      ...
      {createOpen ? <div className="avv-modal-layer" role="dialog" aria-modal="true" aria-labelledby="lemma-create-title" onMouseDown={(event) => { if (event.target === event.currentTarget && !busy) setCreateOpen(false); }}><section className="avv-modal-card"><p className="avv-eyebrow">Manual mapping</p><h2 id="lemma-create-title">Thêm lemma override</h2><label>Original word<input autoFocus value={draft.originalWord} maxLength={200} onChange={(event) => setDraft((current) => ({ ...current, originalWord: event.target.value }))} placeholder="VD: phở" /></label><label>Lemma (canonical form)<input value={draft.lemma} maxLength={200} onChange={(event) => setDraft((current) => ({ ...current, lemma: event.target.value }))} placeholder="VD: phở" /></label><label>POS (tuỳ chọn)<select value={draft.posTag} onChange={(event) => setDraft((current) => ({ ...current, posTag: event.target.value }))}><option value="">Để lemmatizer phân loại</option>{LEMMA_POS_TAGS.filter(Boolean).map((tag) => <option key={tag} value={tag}>{tag}</option>)}</select></label><label>Notes (tuỳ chọn)<textarea rows={3} value={draft.notes} onChange={(event) => setDraft((current) => ({ ...current, notes: event.target.value }))} placeholder="Lý do override…" /></label><div className="avv-modal-actions"><button className="btn-secondary" type="button" disabled={busy} onClick={() => setCreateOpen(false)}>Hủy</button><button className="btn-primary" type="button" disabled={busy} onClick={() => void createOverride()}>{busy ? 'Đang xác minh…' : 'Lưu override'}</button></div></section></div> : null}
      {deleteRow ? <div className="avv-modal-layer" role="dialog" aria-modal="true" aria-labelledby="lemma-delete-title" onMouseDown={(event) => { if (event.target === event.currentTarget && !busy) setDeleteRow(null); }}><section className="avv-modal-card"><p className="avv-eyebrow">Xác nhận thay đổi</p><h2 id="lemma-delete-title">Xoá override?</h2><p>Lemmatizer sẽ quay lại cơ chế mặc định cho từ này sau khi backend reload mapping.</p><strong className="avv-mono-copy">{deleteRow.originalWord} → {deleteRow.lemma}</strong><div className="avv-modal-actions"><button className="btn-secondary" type="button" disabled={busy} onClick={() => setDeleteRow(null)}>Hủy</button><button className="btn-danger" type="button" disabled={busy} onClick={() => void removeOverride()}>{busy ? 'Đang xác minh…' : 'Xoá override'}</button></div></section></div> : null}
    </main>
  );
}
