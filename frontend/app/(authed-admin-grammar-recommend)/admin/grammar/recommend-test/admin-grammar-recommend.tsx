...
export function AdminGrammarRecommend() {
  ...
  return <main className="grt-next-shell">
    ...
    <form className="grt-next-form" onSubmit={(event) => { event.preventDefault(); void run(); }}><label htmlFor="grt-next-issue">Mô tả lỗi grammar</label><textarea id="grt-next-issue" maxLength={2000} value={issue} onChange={(event) => setIssue(event.target.value)} placeholder="Ví dụ: học viên dùng sai thì khi kể một câu chuyện đã kết thúc" /><div className="grt-next-presets" aria-label="Mẫu kiểm thử">{presets.map((item) => <button type="button" key={item} onClick={() => { setIssue(item); void run(item); }}>{item}</button>)}</div><div className="grt-next-actions"><button className="btn-primary" type="submit" disabled={loading}>{loading ? 'Đang chạy matcher…' : 'Chạy matcher'}</button><button className="btn-secondary" type="button" onClick={clear}>Xóa kết quả</button><span>{issue.length}/2000</span></div></form>
