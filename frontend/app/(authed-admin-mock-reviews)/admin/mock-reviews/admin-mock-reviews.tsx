...
export function AdminMockReviews({ examId, embedded }: { examId: string; embedded: boolean }) {
  ...
  return <main className={`mrr-shell${embedded ? ' is-embedded' : ''}`}>
    {!embedded && <header className="mrr-hero"><div><p className="mrr-kicker">Vận hành Mock Test</p><h1>Duyệt bài & công bố</h1><p>Nhận hồ sơ → đối chiếu từng kỹ năng → chốt band → công bố kết quả canonical.</p></div></header>}
    ...
    {detail && <section className="mrr-detail">
      <div className="mrr-detail-head"><button className="adm-btn-secondary" type="button" disabled={Boolean(busy)} onClick={closeDetail}>← Bảng lớp</button><div><span className={`mrr-status is-${detail.review.status}`}>{STATUS[detail.review.status] || detail.review.status}</span><h2>{detail.sitting.studentName}</h2><p>Hồ sơ <code>{detail.review.id.slice(0, 8)}</code> · lượt thi <code>{detail.sitting.id.slice(0, 8)}</code></p></div><div className="mrr-detail-actions">{detail.review.status === 'queued' && <button className="adm-btn-primary" disabled={Boolean(busy)} onClick={() => void reconcileDetailMutation('claim', () => window.api.post(`/admin/mock-reviews/${encodeURIComponent(detail.review.id)}/claim`, {}), (value) => value.review.status !== 'queued' && value.review.claimedBy === profile.id, 'Đã nhận hồ sơ.')}>Nhận duyệt</button>}{['claimed', 'edited'].includes(detail.review.status) && <button className="adm-btn-secondary" disabled={Boolean(busy)} onClick={() => void reconcileDetailMutation('unclaim', () => window.api.post(`/admin/mock-reviews/${encodeURIComponent(detail.review.id)}/release-claim`, {}), (value) => value.review.status === 'queued' && !value.review.claimedBy, 'Đã trả hồ sơ về hàng chờ.')}>Bỏ nhận</button>}</div></div>
      ...
      <section className="mrr-band-card"><div><p className="mrr-kicker">Quyết định của giám khảo</p><h3>Band cuối</h3><p>Band gợi ý có provenance; Overall chính thức luôn do backend tính lại.</p></div><div className="mrr-band-grid">{bandSkills.map((skill) => <label key={skill}><span>{LABEL[skill]}</span><input type="number" min="0" max="9" step="0.5" value={bandDraft[skill] || ''} disabled={detail.review.status === 'released'} onChange={(event) => setBandDraft({ ...bandDraft, [skill]: event.target.value })}/><small>{detail.blankableSkills.includes(skill) ? 'Không có band quy đổi; có thể để trống.' : SOURCE[skill]}</small>{detail.requiredSkills.includes(skill) && <em><input type="checkbox" checked={flagDraft[skill] === true} disabled={detail.review.status === 'released'} onChange={(event) => setFlagDraft({ ...flagDraft, [skill]: event.target.checked })}/> Cần test lại</em>}</label>)}</div><p className="mrr-overall">Overall xem trước <strong>{preview == null ? '—' : preview.toFixed(1)}</strong></p><label className="mrr-comment"><span>Nhận xét tổng cho học viên</span><textarea rows={4} value={comment} disabled={detail.review.status === 'released'} onChange={(event) => setComment(event.target.value)}/></label><div className="mrr-publish-row"><button className="adm-btn-secondary" type="button" disabled={Boolean(busy) || detail.review.status === 'released'} onClick={() => void saveBands()}>{busy === 'save' ? 'Đang đối chiếu…' : 'Lưu band cuối'}</button><select aria-label="Kênh công bố" value={channel} disabled={Boolean(busy) || detail.review.status === 'released'} onChange={(event) => setChannel(event.target.value)}><option value="in_app">In-app</option><option value="email">Email</option><option value="manual">Thủ công</option></select><button className="adm-btn-primary" type="button" disabled={Boolean(busy) || detail.review.status !== 'reviewed'} onClick={() => setReleaseTarget({ kind: 'one', id: detail.review.id, name: detail.sitting.studentName })}>Công bố kết quả</button>{['reviewed', 'released'].includes(detail.review.status) && <a className="adm-btn-secondary" target="_blank" rel="noreferrer" href={`/admin/mock-reviews/report?review_id=${encodeURIComponent(detail.review.id)}&mock_exam_id=${encodeURIComponent(examId)}`}>Xem phiếu điểm ↗</a>}</div><p className="mrr-caveat">Công bố chỉ mở sau khi hồ sơ ở trạng thái “Đã duyệt”. Backend tiếp tục kiểm tra Writing đã duyệt và quyền claim trước khi mở khoá kết quả.</p></section>
    </section>}
