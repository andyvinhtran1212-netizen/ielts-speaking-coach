...
(function () {
    ...
    tbody.innerHTML = items.map(function (it) {
      ...
      if (it.library === 'l3_test' && it.slug) {
        actions +=
          ...
          // Public visibility is independent from mock/class assignment. This
          // rollback surface must write the same canonical flag as the native UI.
          ' <button type="button" class="ar-row-action" ' +
            'data-action="toggle-visibility" data-test-id="' + escapeHtml(it.slug) + '" ' +
            'data-next="' + (it.is_public ? '0' : '1') + '">' +
            (it.is_public ? 'Ẩn khỏi web' : 'Mở công khai') + '</button>' +
  ...
  // Toggle the canonical public flag; mock/class relationships stay untouched.
  function handleToggleVisibility(btn) {
    var testId = btn.getAttribute('data-test-id');
    var next = btn.getAttribute('data-next') === '1';
    var msg = next
      ? 'Mở "' + testId + '" công khai trên web?\n\nMọi học viên sẽ thấy và tự luyện được đề này.'
      : 'Ẩn "' + testId + '" khỏi web?\n\nĐề vẫn dùng được trong mock test và bài giao lớp.';
    if (!window.confirm(msg)) return;
    btn.disabled = true;
    window.api.patch('/admin/reading/content/tests/' + encodeURIComponent(testId) + '/visibility',
                     { is_public: next })
      .then(function () { setStatus(next ? 'Đã mở công khai.' : 'Đã ẩn khỏi web.', 'ok'); loadList(); })
      .catch(function (e) {
        btn.disabled = false;
  ...
  function handleListClick(ev) {
    ...
    if (action === 'toggle-visibility') return handleToggleVisibility(btn);
