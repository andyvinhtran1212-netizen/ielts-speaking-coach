  ...
  const students = payload.students.map((item) => {
    ...
    return {
      ...
      score: finite(row.score), flags: Array.isArray(row.flags) ? row.flags.map((flag) => {
        const f = object(flag);
        return { severity: text(f.severity), label: text(f.label), why: text(f.why), action: text(f.action) };
...
export function normalizeEffort(value) {
  ...
  return {
    ...
    axes: payload.axes.map((item) => {
      const row = object(item);
      return { axis: text(row.axis), wrong: Math.max(0, finite(row.wrong) || 0), median_sec: finite(row.median_sec) };
    }).filter((row) => row.axis),
  };
}
