...
await page.goBack();
await page.getByText('Older Search Student', { exact: true }).waitFor();
check('Back khôi phục từ khóa và kết quả Queue', new URL(page.url()).searchParams.get('q') === 'OLD-201' && await page.getByLabel('Tìm học viên').inputValue() === 'OLD-201');
await page.goForward();
await page.getByText('Learner Two', { exact: true }).waitFor();
check('Forward khôi phục bộ lọc trước đó', !new URL(page.url()).searchParams.has('q') && await page.getByLabel('Tìm học viên').inputValue() === '');
...
mockRows = Array.from({ length: 51 }, (_, index) => base(`sized-${index + 1}`, index === 50 ? 'Fifty-first Pending' : `Sized Pending ${index + 1}`, 'pending', { sitting_id: `sit-sized-${index + 1}` }));
await page.goto(`${BASE}/admin/writing/queue?embed=1&mocklane=1&queue_status=pending`, { waitUntil: 'domcontentloaded' });
await page.getByText('Sized Pending 1', { exact: true }).waitFor();
const fiftyRead = page.waitForResponse((response) => {
  const url = new URL(response.url());
  return url.pathname === QUEUE_PATH && url.searchParams.get('limit') === '50' && url.searchParams.get('offset') === '0';
});
await page.getByLabel('Số dòng').selectOption('50');
await fiftyRead;
check('cỡ trang 50 được lưu vào URL và request canonical', new URL(page.url()).searchParams.get('page_size') === '50');
await page.getByRole('button', { name: 'Trang sau' }).click();
await page.getByText('Fifty-first Pending', { exact: true }).waitFor();
check('trang 2 dùng offset 50 và giữ cỡ trang', new URL(page.url()).searchParams.get('page') === '2' && new URL(page.url()).searchParams.get('page_size') === '50' && requests.some((item) => item.path === QUEUE_PATH && item.search.includes('limit=50') && item.search.includes('offset=50')));
await page.reload({ waitUntil: 'domcontentloaded' });
await page.getByText('Fifty-first Pending', { exact: true }).waitFor();
check('reload giữ nguyên 50 dòng/trang và trang 2', await page.getByLabel('Số dòng').inputValue() === '50' && new URL(page.url()).searchParams.get('page') === '2');
await page.getByLabel('Số dòng').selectOption('25');
await page.waitForURL((url) => !url.searchParams.has('page') && !url.searchParams.has('page_size'));
await page.goBack();
await page.getByText('Fifty-first Pending', { exact: true }).waitFor();
check('đổi cỡ trang reset về trang 1; Back khôi phục trang 2 cỡ 50', new URL(page.url()).searchParams.get('page') === '2' && new URL(page.url()).searchParams.get('page_size') === '50' && await page.getByLabel('Số dòng').inputValue() === '50');
await page.locator('tr', { hasText: 'Fifty-first Pending' }).getByRole('button', { name: 'Xem tiến trình' }).click();
await page.waitForURL((url) => url.pathname === '/admin/writing/status');
const statusUrl = new URL(page.url());
check('Queue mở Status với toàn bộ context URL', statusUrl.searchParams.get('from') === 'queue' && statusUrl.searchParams.get('mocklane') === '1' && statusUrl.searchParams.get('embed') === '1' && statusUrl.searchParams.get('queue_status') === 'pending' && statusUrl.searchParams.get('page') === '2' && statusUrl.searchParams.get('page_size') === '50');
await page.getByRole('link', { name: 'Quay lại Queue' }).click();
await page.getByText('Fifty-first Pending', { exact: true }).waitFor();
check('Status quay lại đúng trang 2 cỡ 50', new URL(page.url()).searchParams.get('page') === '2' && new URL(page.url()).searchParams.get('page_size') === '50');
