...
export function validateListeningPreviousPhaseHandoff(input) {
  ...
  if (runMetadata?.workflowName !== 'Listening Gate E coexistence drill' ||
  ...
  if (!e || e.schema_version !== 1 || e.drill_id !== 'gate-e-listening-coexistence-v1' ||
...
function findEvidence(directory) {
  for (const entry of readdirSync(directory, { withFileTypes: true })) {
    ...
    else if (entry.name === 'gate-e-listening-coexistence-evidence.json') return target;
  }
  return null;
}
...
function run() {
  ...
  const output = path.resolve('test-results/gate-e-listening-coexistence-handoff.json');
  let result;
  try {
    ...
    result = validateListeningPreviousPhaseHandoff({
