...
export function validateDictationPreviousPhaseHandoff(input) {
  ...
  if (runMetadata?.workflowName !== 'Dictation Gate E coexistence drill' ||
  ...
  if (!e || e.schema_version !== 1 || e.drill_id !== 'gate-e-dictation-coexistence-v1' ||
...
function findEvidence(directory) {
  for (const entry of readdirSync(directory, { withFileTypes: true })) {
    ...
    else if (entry.name === 'gate-e-dictation-coexistence-evidence.json') return target;
  }
  return null;
}
...
function run() {
  ...
  const output = path.resolve('test-results/gate-e-dictation-coexistence-handoff.json');
  let result;
  try {
    ...
    result = validateDictationPreviousPhaseHandoff({
