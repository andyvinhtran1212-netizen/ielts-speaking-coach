...
const manifest = readJson('tooling/gate-e-listening-device-matrix.json');
...
if (configuredRunnerImage && configuredRunnerImage !== manifest.ci_runner) {
  throw new Error(
    `Listening matrix runner ${configuredRunnerImage} != manifest ${manifest.ci_runner}`,
  );
}
...
if (manifest.playwright_version !== installedPlaywright) {
  throw new Error(
    `Listening matrix Playwright ${manifest.playwright_version} != lockfile ${installedPlaywright}`,
  );
}
...
for (const project of manifest.automated_projects) {
  ...
  if (!target) throw new Error(`Listening matrix target missing: ${project.engine}`);
...
console.log(`Listening Gate E matrix pins verified: ${manifest.matrix_id}`);
