// Fixture-backed browser contract for the native, read-only Reading admin hub.
...
const adminId = '00000000-0000-0000-0000-000000000115';
const session = JSON.stringify({ access_token: 'admin-reading-not-real', refresh_token: 'x', expires_at: Math.floor(Date.now() / 1000) + 3600, user: { id: adminId, email: 'admin-reading@local' } });
const results = [];
...
await page.route('**/*', async (route) => {
  ...
  if (parsed.pathname === '/auth/me') return json({ id: adminId, email: 'admin-reading@local', role: 'admin' });
  return json({});
});

await page.goto(`${BASE}/admin/reading`, { waitUntil: 'domcontentloaded' });
await page.getByRole('heading', { name: 'Reading workspace', exact: true }).waitFor();
...
await page.getByRole('heading', { name: 'Reading workspace', exact: true }).waitFor();
check('desktop ba cột và không tràn ngang', await page.evaluate(() => getComputedStyle(document.querySelector('.rdh-grid')).gridTemplateColumns.split(' ').length === 3 && document.documentElement.scrollWidth <= innerWidth));
await page.emulateMedia({ colorScheme: 'dark', reducedMotion: 'reduce' });
await page.reload({ waitUntil: 'domcontentloaded' });
await page.getByRole('heading', { name: 'Reading workspace', exact: true }).waitFor();
check('dark mode vẫn phân biệt card và text', await page.evaluate(() => { const style = getComputedStyle(document.querySelector('.rdh-card')); return style.backgroundColor !== style.color && style.color !== 'rgba(0, 0, 0, 0)'; }));
check('không có write ngoài contract', writes.length === 0, writes.join(', '));
check('không có lỗi JS', errors.length === 0, errors.join(' | '));

await browser.close();
const failed = results.filter((item) => !item.ok);
console.log(`\nAdmin Reading hub flow: ${results.length - failed.length}/${results.length} checks passed`);
