...
await page.route('**/*', async (route) => {
  ...
  if (parsed.pathname === '/admin/listening/content/c1' && method === 'GET') {
    contentReads += 1;
    return json({ id: 'c1', title: 'Detail <script>', status, source_type: 'upload_mp3', accent_tag: 'uk_rp', cefr_level: 'B2', ielts_section: 2, audio_duration_seconds: 75, audio_storage_path: 'audio/c1.mp3', audio_signed_url: null, transcript: 'Hostile <img onerror=alert(1)> stays text.', topic_tags: ['transport'], external_license: 'CC-BY', external_source_url: 'https://example.test/source', updated_at: '2026-08-14T00:00:00Z' });
  }
  if (parsed.pathname === '/admin/listening/exercises' && parsed.searchParams.get('content_id') === 'c1') return json({ detail: 'exercise lookup unavailable' }, 503);
