...
const manifest = readJson('tooling/gate-e-reading-device-matrix.json');
...
if (configuredRunnerImage && configuredRunnerImage !== manifest.ci_runner) {
  throw new Error(
    `Reading matrix runner ${configuredRunnerImage} != manifest ${manifest.ci_runner}`,
  );
}
...
if (manifest.playwright_version !== installedPlaywright) {
  throw new Error(
    `Reading matrix Playwright ${manifest.playwright_version} != lockfile ${installedPlaywright}`,
  );
}
...
for (const project of manifest.automated_projects) {
  ...
  if (!target) throw new Error(`Reading matrix target missing: ${project.engine}`);
...
console.log(`Reading Gate E matrix pins verified: ${manifest.matrix_id}`);
