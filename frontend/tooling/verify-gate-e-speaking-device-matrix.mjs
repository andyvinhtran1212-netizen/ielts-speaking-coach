...
const manifest = readJson('tooling/gate-e-speaking-device-matrix.json');
...
if (manifest.playwright_version !== installedPlaywright) {
  throw new Error(
    `Speaking matrix Playwright ${manifest.playwright_version} != lockfile ${installedPlaywright}`,
  );
}
...
for (const project of manifest.automated_projects) {
  ...
  if (!target) throw new Error(`Speaking matrix target missing: ${project.engine}`);
...
console.log(`Speaking Gate E matrix pins verified: ${manifest.matrix_id}`);
