...
export function resolveReadingPushPhase({ sourceSha, admission, inputs }) {
  ...
  if (admission !== 'legacy') throw new Error('reading-admission-invalid');
...
function run() {
  ...
  const inputs = JSON.parse(readFileSync(
    path.resolve('tooling/gate-e-reading-coexistence-push-inputs.json'), 'utf8',
  ));
  const resolved = resolveReadingPushPhase({
    sourceSha,
    admission: CORE_PLAYER_AFFINITY_POLICY.surfaces.reading_exam.admit_new,
    inputs,
  });
