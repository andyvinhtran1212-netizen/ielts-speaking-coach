...
await page.goto(`${BASE}/admin/writing/new?student_id=s1`);
...
check('admin gate và preset học viên canonical', preset === 's1' && requests.some((x) => x.path === '/auth/me') && requests.some((x) => x.path === '/admin/students'), `preset=${preset}`);
