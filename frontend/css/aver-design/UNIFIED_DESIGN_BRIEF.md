...
The redesign program runs in phases. Phases 1–3 are complete (13 pages); Phase 4 admin cluster is **STRUCTURALLY COMPLETE** (8 sub-pages fully migrated + `admin.html` chrome-only via Sprint 6.14d-α, with utility-class refactor + per-tab polish deferred to 6.14d-β/γ per DESIGN_SYSTEM.md § 14.5.2/14.5.3 triggers). Grammar Wiki cluster remains.
...
| **Phase 4** — Marketing + Admin + Grammar Wiki | **Marketing COMPLETE:** `index.html` surgical migration + multi-skill repositioning (Sprint 6.13a + 6.13a-extension, PRs #145/#146); `pricing.html` (6.13b, PR #147); Era B `landing.html` duplicate eliminated atomically (Sprint 6.13a). **Admin cluster STRUCTURALLY COMPLETE:** small writing cluster — `admin-writing.html` + `admin-writing-new.html` + `admin-writing-status.html` + `admin-writing-prompts.html` (6.14a, PR #149); table pages — `admin-writing-assignments.html` + `admin-students.html` (6.14b, PR #150); instructor queue + grading — `admin-instructor-queue.html` + `admin-writing-grade.html` (6.14c, PR #151); `admin.html` monolith chrome-only (6.14d-α, PR #153 — 8/9 admin pages FULLY MIGRATED + 1/9 STRUCTURALLY COMPLETE per § 14.5.1); pre-work PR #148 + audit hotfix PR #152. **Deferred with un-defer triggers:** Sprint 6.14d-β Tailwind utility-class refactor + 6.14d-γ per-tab primitive adoption polish for `admin.html` (see DESIGN_SYSTEM.md § 14.5.2 / § 14.5.3). **UPCOMING:** Sprint 6.15 Grammar Wiki cluster (`grammar.html` + 5 sub-pages, `vocab-article.html`; decision pending on DM Sans + Lora sub-system vs unification). | **IN PROGRESS** |
...
**24 pages redesigned cumulative** (Phase 1: 4 + Phase 2: 3 + Phase 3: 6 + Phase 4 marketing: 2 + Phase 4 admin: 9). Phase 4 admin cluster STRUCTURALLY COMPLETE (8 fully migrated + 1 chrome-only via 6.14d-α, β/γ deferred with triggers).
