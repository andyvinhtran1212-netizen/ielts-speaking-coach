...
| 4 | `frontend/index.html` | 6.13a | #145 | **Phase 4 opening — marketing landing.** Surgical migration on the canonical landing/login entry (667-line page; ~57-line inline `<style>` + Tailwind palette config). At the frontend root. Drops Inter for Plus Jakarta Sans + JetBrains Mono; replaces 37 Tailwind palette + inline hex literals with `--av-*` tokens. Conversion flow byte-identical: 8 CTAs to `/login.html`, 2 to `/grammar.html`, 1 inline to `/pricing.html`, 1 footer to `/frontend/pages/home.html`, 3 section anchors (`#features`, `#how-it-works`, `#pricing` id-only). Pricing section keeps inline `style="display:none"` pre-launch hide. Hero + final CTA + footer are intentionally "always-dark" atmosphere (whitelist `color: #FFFFFF` per Gate 4). **Atomic Era B reconciliation:** `frontend/landing.html` (off-palette `#1B3A5C`/`#0D7377` orphan with zero production consumers) deleted in same PR; sentinel test pins deletion + no-resurrection. Pre-work falsified 5 marketing-specific risks: NO SEO meta tags, NO analytics, NO A/B infra, NO cookie banner, NO auth-state-aware CTA — index.html is currently a pure static splash. |
...
| Phase 4 | 6.13 – 6.15 | **In progress.** Sprint 6.13a: `index.html` ✅ (also deletes Era B `landing.html` orphan). Sprint 6.13b: `pricing.html`. Sprint 6.14: `dashboard.html` + `admin.html` cluster. Sprint 6.15: Grammar Wiki cluster. |
