...
import { validateListeningPreviousPhaseHandoff } from '../tooling/validate-gate-e-listening-coexistence-handoff.mjs';
import { resolveListeningPushPhase } from '../tooling/resolve-gate-e-listening-push-phase.mjs';

const FRONTEND = path.dirname(path.dirname(fileURLToPath(import.meta.url)));
const ROOT = path.dirname(FRONTEND);
const read = (p) => readFileSync(path.join(ROOT, p), 'utf8');
const manifest = JSON.parse(read('frontend/tooling/gate-e-listening-coexistence-drill.json'));
const workflow = read('.github/workflows/listening-coexistence-drill.yml');
const spec = read('frontend/tests/listening-coexistence-drill/staging-listening-coexistence.spec.js');
const config = read('frontend/playwright.listening-coexistence.config.js');
...
describe('Listening coexistence drill contract', () => {
  test('pins ordered floor, cutover and rollback phases', () => {
    assert.equal(manifest.drill_id, 'gate-e-listening-coexistence-v1');
  ...
  test('workflow is staging-bound, serial and always uploads provenance', () => {
    ...
    assert.match(workflow, /GITHUB_EVENT_NAME" = push[\s\S]*?resolve-gate-e-listening-push-phase\.mjs/);
    ...
    assert.match(workflow, /runner\.temp \}\}\/gate-e-listening-preflight/);
    assert.match(workflow, /Verify complete phase evidence bundle\n\s+if: always\(\)/);
    assert.match(workflow, /test -f "\$EVIDENCE_DIR\/gate-e-listening-coexistence-lineage\.json"/);
    assert.match(workflow, /test -f "\$EVIDENCE_DIR\/gate-e-listening-coexistence-handoff\.json"/);
    assert.ok(workflow.indexOf('Preserve verified preflight evidence') <
      workflow.indexOf('npx playwright test -c playwright.listening-coexistence.config.js'));
  ...
  test('push resolver derives floor, cutover and rollback from policy + checked handoff', () => {
    ...
    assert.equal(resolveListeningPushPhase({ sourceSha: FLOOR, admission: 'legacy',
      inputs: empty }).phase, 'floor');
    const cutover = { ...empty, rollback_floor_sha: FLOOR, previous_phase_run_id: '123',
      previous_legacy_attempt_id: LEGACY, previous_legacy_test_id: 'LISTEN-1' };
    assert.equal(resolveListeningPushPhase({ sourceSha: CUTOVER, admission: 'next',
      inputs: cutover }).phase, 'cutover');
    const rollback = { ...empty, rollback_floor_sha: FLOOR, previous_phase_run_id: '124',
      previous_next_attempt_id: NEXT, previous_next_test_id: 'LISTEN-2' };
    assert.equal(resolveListeningPushPhase({ sourceSha: ROLLBACK, admission: 'legacy',
      inputs: rollback }).phase, 'rollback');
    assert.throws(() => resolveListeningPushPhase({ sourceSha: CUTOVER, admission: 'next',
      inputs: empty }), /cutover-push-input-invalid/);
  });

  test('browser evidence proves admission, persisted affinity and stable reopen', () => {
    assert.match(spec, /core-player\/launch\?surface=listening_test/);
    assert.match(spec, /expectedPath === '\/listening\/test\/session'/);
    assert.match(spec, /getByRole\('button', \{ name: 'Bắt đầu lại từ đầu' \}\)/);
    assert.match(spec, /getByRole\('button', \{ name: 'Bắt đầu test' \}\)/);
    assert.match(spec, /page\.once\('dialog', \(dialog\) => dialog\.accept\(\)\)/);
    assert.match(spec, /page\.locator\('#btn-start'\)/);
    ...
    assert.match(spec, /Next Listening prestart controls did not become ready/);
    ...
    assert.match(spec, /api\/listening\/tests\?test_type=full/);
    assert.match(spec, /\?id=\$\{encodeURIComponent\(testId\)\}/);
  ...
  test('handoff is bound to prior successful artifact and attempt/test pair', () => {
    const metadata = { workflowName: 'Listening Gate E coexistence drill', event: 'workflow_dispatch',
    ...
    const evidence = { schema_version: 1, drill_id: manifest.drill_id, phase: 'floor',
      ...
      created_attempt_id: LEGACY, created_test_id: 'LISTEN-1' };
    assert.equal(validateListeningPreviousPhaseHandoff({ phase: 'cutover', sourceSha: CUTOVER,
      floorSha: FLOOR, previousRunId: '123', previousLegacyAttemptId: LEGACY,
      previousLegacyTestId: 'LISTEN-1', previousNextAttemptId: '', previousNextTestId: '',
    ...
    const cutover = { ...evidence, phase: 'cutover', expected_admission: 'next',
      ...
      created_attempt_id: NEXT, created_test_id: 'LISTEN-2' };
    assert.equal(validateListeningPreviousPhaseHandoff({ phase: 'rollback', sourceSha: ROLLBACK,
      floorSha: FLOOR, previousRunId: '124', previousLegacyAttemptId: '', previousLegacyTestId: '',
      previousNextAttemptId: NEXT, previousNextTestId: 'LISTEN-2', runMetadata: metadata,
      previousEvidence: cutover, isAncestor: (a, d) => a === CUTOVER && d === ROLLBACK }).verified, true);
    assert.throws(() => validateListeningPreviousPhaseHandoff({ phase: 'cutover', sourceSha: CUTOVER,
      floorSha: FLOOR, previousRunId: '123', previousLegacyAttemptId: LEGACY,
    ...
    assert.throws(() => validateListeningPreviousPhaseHandoff({ phase: 'cutover',
      sourceSha: CUTOVER, floorSha: FLOOR, previousRunId: '123',
      previousLegacyAttemptId: LEGACY, previousLegacyTestId: 'LISTEN-1',
