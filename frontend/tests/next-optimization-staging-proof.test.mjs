...
describe('Next optimization staging proof safety', () => {
  ...
  test('warms one exact cache key before PATCH and polls the same URL', () => {
    assert.match(SPEC, /const category = `e2e-cache-topic-\$\{suffix\}`\.toLowerCase\(\)/);
  ...
  test('is staging-only and cleans the exact created card and topic even after failure', () => {
    ...
    assert.match(SPEC, /request\.post\(`\$\{STAGING_API\}\/admin\/content-topics`/);
    assert.match(SPEC, /if \(importAttempted && !createdId\) createdId = await findProbeId\(\)/);
    assert.match(SPEC, /if \(topicCreateAttempted && !createdTopicId\) createdTopicId = await findProbeTopicId\(\)/);
    assert.match(SPEC, /admin\/content-topics\/\$\{encodeURIComponent\(createdTopicId\)\}/);
    assert.match(SPEC, /new AggregateError\(cleanupFailures/);
