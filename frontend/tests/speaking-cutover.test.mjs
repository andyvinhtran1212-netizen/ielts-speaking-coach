/**
 * speaking-cutover.test.mjs — `/speaking` là route canonical (cutover 2026-08-05).
 *
 ...
 * `/pages/grammar-article.html` (vẫn 200 tới hôm nay), và `/grammar.html`, `/pages/home.html` cũng
 * thế sau các cutover trước.
 *
 * Lý do giữ legacy không chỉ là tương thích bookmark: chừng nào CẢ HAI bản còn
 * sống thì cổng parity G1 còn so được — và với `/speaking` thì lớp đó vừa mới dựng
 * xong (authed-G1 + ba lớp bằng chứng, PR #930–#939) nên vứt đi ngay là phí nhất. Việc gỡ thuộc Phase 7.
 *
 ...
 *
 * Hiện KHÔNG có ngoại lệ nào: `public/pages/speaking.html` không hề tự trỏ vào
...
const SKIP_DIRS = new Set([
  ...
  // Fixture của test backend chứa `"url": "/pages/speaking.html"` — đó là DỮ LIỆU
...
// đúng ba dạng đã gặp thật khi sweep: `/pages/speaking.html`, `pages/home.html`
// (admin.html, onboarding.html) và `home.html` (flashcard-study.js).
const LINK_SRC = '["\'`](?:\\.\\./|/)?(?:pages/)?speaking\\.html(?:[?#][^"\'`]*)?["\'`]';
...
describe('cutover /speaking — điều hướng canonical', () => {
  test('không tệp nào ngoài danh sách được phép còn LINK tới speaking.html', () => {
    const offenders = [];
  ...
  test('chrome dùng chung trỏ route canonical', () => {
    ...
    assert.ok((body.match(/["']\/speaking["']/g) || []).length >= 2,
      'cần đủ 3 chỗ: logo, tab Trang chủ, và luật prefetch href_matches');
  });

  test('KHÔNG rewrite /speaking và KHÔNG redirect /pages/speaking.html', () => {
    // Hai chốt khác nhau, đừng gộp:
    //  · rewrite `/speaking` → bản legacy: nếu còn thì route Next bị CHE và cutover
    //    chỉ là hình thức. Cổng route-ownership cũng chặn, nhưng nó chỉ chạy khi
    //    build — chốt ở đây đọc được ngay.
    //  · redirect `/pages/speaking.html` → `/speaking`: nghe rất hợp lý mà lại phá cổng.
    //    Hai vế sẽ cùng dừng ở `/speaking`, chốt `same-final-url` từ chối, và trang
    //    chủ mất luôn lớp parity vừa dựng xong (authed-G1 + ba lớp bằng chứng, PR #930–#939).
    //
    ...
    assert.ok(!/source:\s*['"`]\/speaking['"`]/.test(nextCfg),
      'rewrite /speaking còn sót = route Next bị che, cutover chỉ là hình thức');
    assert.ok(!/source:\s*['"`]\/pages\/speaking\.html['"`]/.test(nextCfg),
      'redirect /pages/speaking.html sẽ gỡ mất lớp parity authed vừa dựng');

    const vercel = JSON.parse(readFileSync(path.join(FRONTEND, 'vercel.json'), 'utf8'));
    const rules = [...(vercel.redirects || []), ...(vercel.rewrites || [])];
    assert.deepEqual(
      rules.filter((r) => ['/speaking', '/pages/speaking.html'].includes(String(r.source || ''))), [],
  ...
  test('route Next tồn tại đúng chỗ', () => {
    ...
    const home = pairs.find((p) => p.name === 'speaking');
    assert.ok(home, 'cặp parity "speaking" phải còn');
    assert.equal(home.next, '/speaking');
    assert.equal(home.legacy, '/pages/speaking.html');
    assert.deepEqual(home.allow || [], [],
      'behavior đã port xong — không còn lý do miễn trừ mục nào');

    assert.ok(statSync(path.join(FRONTEND, 'app/(authed-speaking)/speaking/page.tsx')).isFile());
  });
