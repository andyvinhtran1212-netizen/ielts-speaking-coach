// Mutation/recovery regression in a real hydrated browser. These
...
test('sealed upload committed before a network failure reconciles and survives reload', async ({ page }) => {
  ...
  const result = await page.evaluate(async ({ sessionId }) => {
    const blob = new Blob(['speaking-regression-original-audio'], { type: 'audio/webm' });
  ...
  expect(uploads[0].authorization).toBe('Bearer speaking-regression-fake-token');
  expect(uploads[0].body).toContain('name="question_id"');
  expect(uploads[0].body).toContain('q1');
  expect(await outgoingAudioPayloads(page)).toEqual(['speaking-regression-original-audio']);
