/** Persisted attempts keep deterministic renderer affinity during recovery. */
...
describe('current admission policy preserves behavior', () => {
  test('policy is internally valid and every historical renderer identity remains explicit', () => {
    ...
    for (const [surface, config] of Object.entries(CORE_PLAYER_AFFINITY_POLICY.surfaces)) {
      ...
      assert.match(config.legacy.path, /^\/pages\/[a-z0-9-]+\.html$/);
    }
