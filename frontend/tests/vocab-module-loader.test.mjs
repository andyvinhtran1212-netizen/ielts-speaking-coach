...
describe('Sprint 7.3 — /js/vocab-landing.js gains TAB_LOADERS module path', () => {
  ...
  test('declares TAB_LOADERS with flashcards + exercises entries', () => {
    // Bind each key DIRECTLY to its dynamic import — order- and distance-
    // independent (My Vocab + Needs Review modules were removed).
    ...
    assert.doesNotMatch(src, /vocab-modules\/my-vocab\.js/);
    assert.doesNotMatch(src, /vocab-modules\/needs-review\.js/);
  });
...
describe('Sprint 7.4 — /js/vocab-modules/flashcards.js module', () => {
  ...
  test('HTML template ships the category chips with data-action="toggle-category"', () => {
    // needs_review chip was removed with the Needs Review feature.
    const cats = ['used_well', 'upgrade_suggested', 'manual'];
    cats.forEach((c) => {
      const re = new RegExp(`data-action="toggle-category"[^>]*data-cat="${c}"`);
      assert.match(src, re, `missing category chip for ${c}`);
    });
    assert.doesNotMatch(src, /data-cat="needs_review"/);
  });
