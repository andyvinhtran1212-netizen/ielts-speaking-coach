...
describe('Perf P2.3 — aver-chrome injects prefetch speculation rules', () => {
  ...
  it('allowlists ONLY read-only dashboards — never stateful/admin pages', () => {
    for (const href of [
      ...
      // [cutover /speaking 2026-08-05] prefetch phải khớp đường người dùng
      // THẬT SỰ bấm, nếu không luật này lặng lẽ mất tác dụng.
      // [cutover /writing/dashboard 2026-08-05] cùng lý do hai dòng trên.
      ...
      // [cutover /vocabulary 2026-08-15] public wiki now has a clean App
      // Router owner; prefetch must follow the same canonical chrome target.
      '/grammar', '/vocabulary',
    ]) {
