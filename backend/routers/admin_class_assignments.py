...
def _resolve_course_bank(cohort_id: str, body: "AssignmentCreate") -> tuple[str, dict]:
    ...
    if (bank.get("meta") or {}).get("pronunciation_requirement") and not pronunciation_sets:
        raise HTTPException(400, "Bộ bài tập chưa đăng ký phần phát âm bắt buộc.")
    try:
...
async def list_course_banks(
    ...
    pronunciation_ready = set()
    for chunk in (ids[i:i + _ID_CHUNK] for i in range(0, len(ids), _ID_CHUNK)):
        rows = (supabase_admin.table("course_pronunciation_sets").select("bank_id")
                .in_("bank_id", chunk).eq("is_active", True).execute().data) or []
        pronunciation_ready.update(row["bank_id"] for row in rows if row.get("bank_id"))
    ...
    for bank in banks:
        ...
        pronunciation_is_ready = bank_id in pronunciation_ready
        items.append({
            "id":                    bank_id,
            "code":                  bank.get("code"),
            "lesson_no":             bank.get("lesson_no"),
...
@router.get("/{cohort_id}/action-log")
async def class_action_log(
    ...
    # vẫn trả nhật ký — thiếu tên còn hơn không có dòng nào.
    names: dict = {}
    students: dict = {}
    try:
        aids = [r["assignment_id"] for r in rows if r.get("assignment_id")]
        if aids:
            names = {a["id"]: a.get("title") for a in _paged(
                supabase_admin, "class_assignments", "id, title",
                lambda q: q.in_("id", list(set(aids))))}
        sids = [r["student_id"] for r in rows if r.get("student_id")]
        if sids:
            students = {s["id"]: s.get("full_name") for s in _paged(
                supabase_admin, "students", "id, full_name",
                lambda q: q.in_("id", list(set(sids))))}
    except Exception as exc:  # noqa: BLE001
