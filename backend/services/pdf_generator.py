...
# LanguageTool grammar-error category → VN label (mirror of _GRAMMAR_CAT_LABELS
# in frontend/pages/result.html — keep in sync).
_GRAMMAR_CAT_LABELS = {
...
def _pron_color_hex(score) -> str:
    """Score → hex colour (mirror of result.html _pronScoreColor: ≥75 / ≥55)."""
    if score is None:
        return "#64748b"
    if score >= 75:
        return "#27ae60"
...
_T = {
    ...
    # Sprint 16.1 — pronunciation pills, phoneme drill-down, structured grammar
    "note_vn":   _ps("note_vn",   fontName=_VI_FONT,   fontSize=9,    textColor=_C["muted"], leading=13),
    "pron_eye":  _ps("pron_eye",  fontName="Helvetica-Bold", fontSize=8, textColor=_C["muted"], leading=11, spaceBefore=4, spaceAfter=3),
    "pill":      _ps("pill",      fontName=_VI_FONT,   fontSize=8,    textColor=_C["muted"], alignment=TA_CENTER, leading=14),
    "ph_word":   _ps("ph_word",   fontName=_VI_FONT_B, fontSize=9.5,  textColor=_C["navy"],  leading=13, spaceBefore=4, leftIndent=8),
    "ph_row":    _ps("ph_row",    fontName=_VI_FONT,   fontSize=9,    textColor=_C["slate"], leading=12, leftIndent=16),
    "ph_ex":     _ps("ph_ex",     fontName=_VI_FONT,   fontSize=8.5,  textColor=_C["muted"], leading=11, leftIndent=16),
    "ph_tip":    _ps("ph_tip",    fontName=_VI_FONT,   fontSize=8.5,  textColor=_C["muted"], leading=11, leftIndent=16, spaceAfter=2),
    "gr_cat":    _ps("gr_cat",    fontName=_VI_FONT_B, fontSize=8.5,  textColor=_C["slate"], leading=12, spaceBefore=3),
    "gr_pair":   _ps("gr_pair",   fontName=_VI_FONT,   fontSize=9,    textColor=_C["slate"], leading=12, leftIndent=8),
    "gr_exp":    _ps("gr_exp",    fontName=_VI_FONT,   fontSize=8.5,  textColor=_C["muted"], leading=11, leftIndent=16, spaceAfter=2),
    "gr_more":   _ps("gr_more",   fontName=_VI_FONT,   fontSize=8.5,  textColor=_C["v_muted"], leading=11, spaceBefore=2),
}
...
def _render_pdf(
    ...
    left_col = [
        ...
        Paragraph("Performance Report · averlearning.com", _T["sub"]),
        Spacer(1, 8),
    ...
    story.append(Spacer(1, 10))
    story.append(Paragraph(f"averlearning.com · IELTS Speaking Coach · Generated {gen_date}", _T["ft_gen"]))
...
# ── Pronunciation pills (parity with result.html _pronResultHtml) ──────────────

def _build_pronunciation_pills(r: dict | None) -> list:
    """4 score pills (Overall/Fluency/Accuracy/Complete) from responses.pronunciation_*.
    Mirrors result.html: shown only when assessment completed; null pills skipped."""
    if not r or r.get("pronunciation_status") != "completed" or r.get("pronunciation_score") is None:
        return []
    spec = [
        ("Overall",  r.get("pronunciation_score")),
        ("Fluency",  r.get("pronunciation_fluency")),
        ("Accuracy", r.get("pronunciation_accuracy")),
        ("Complete", r.get("pronunciation_completeness")),
    ]
    cells = [
        Paragraph(
            f'{label}<br/><font size="14" color="{_pron_color_hex(val)}"><b>{val:.0f}</b></font>',
            _T["pill"],
        )
        for label, val in spec if val is not None
    ]
    if not cells:
        return []
    w = _COL_W / len(cells)
...
def _build_phoneme_section(r: dict | None) -> list:
    """Weak-word phoneme breakdown from the raw pronunciation_payload. Mirrors the
    result.html accordion (rendered statically/expanded for print). Legacy pre-15.1
    sessions render a placeholder; empty/no-recognition payloads render nothing."""
    if not r:
        return []
    payload = r.get("pronunciation_payload")
    if not payload:
        return []
    ex = extract_weak_words_from_payload(payload)
    out: list = []

    if ex["weak_words"]:
        out.append(Paragraph("PHÂN TÍCH PHÁT ÂM CHUYÊN SÂU", _T["lbl"]))
        for w in ex["weak_words"]:
            phs = sorted(
                w["phonemes"],
                key=lambda p: 999 if p["score"] is None else p["score"],
            )
            weak_n = sum(1 for p in phs if p["score"] is not None and p["score"] < 70)
            count = f"{weak_n} âm cần luyện" if weak_n > 0 else f"{len(phs)} âm"
            out.append(Paragraph(
                f'{_esc(w["word"])} <font color="#94a3b8">· {count}</font>', _T["ph_word"]
            ))
            for p in phs:
                ref       = PHONEME_REF.get(p["symbol"])
                hexc      = _pron_color_hex(p["score"])
                ipa       = f' /{_esc(ref["ipa"])}/' if ref else ""
                score_txt = "—" if p["score"] is None else f'{round(p["score"])}'
                out.append(Paragraph(
                    f'<b>{_esc(p["symbol"])}</b>{ipa} — <font color="{hexc}">{score_txt}/100</font>',
                    _T["ph_row"],
                ))
                if ref:
                    examples = ", ".join(f"<b>{_esc(x)}</b>" for x in ref["examples"])
                    out.append(Paragraph(f"Ví dụ: {examples}", _T["ph_ex"]))
                    out.append(Paragraph(_esc(ref["tip_vn"]), _T["ph_tip"]))
                else:
                    out.append(Paragraph("Hướng dẫn cho âm này đang được cập nhật.", _T["ph_tip"]))
        out.append(Spacer(1, 6))

    elif ex["legacy"]:
        out.append(Paragraph("PHÂN TÍCH PHÁT ÂM CHUYÊN SÂU", _T["lbl"]))
        out.append(Paragraph(
            "Phân tích phát âm chuyên sâu chưa khả dụng cho phiên này — "
            "các phiên luyện tập gần đây sẽ có nhận xét chi tiết hơn.",
            _T["note_vn"],
        ))
        out.append(Spacer(1, 6))
...
# ── Structured grammar errors (parity with result.html _persistedGrammarBlock) ─

def _build_grammar_section(fb: dict | None) -> list:
    """LanguageTool structured errors (feedback.grammar_check), grouped by category.
    Mode-independent — mirrors result.html which renders it below the transcript."""
    gc = (fb or {}).get("grammar_check")
    if not isinstance(gc, dict):
        return []
    errors = gc.get("errors")
    ...
    out: list = [Paragraph("GRAMMAR ISSUES (LANGUAGETOOL)", _T["lbl"])]
    for cat in order:
        out.append(Paragraph(_esc(_GRAMMAR_CAT_LABELS.get(cat, cat)), _T["gr_cat"]))
        for e in groups[cat]:
            orig = _esc(e.get("original_text") or "")
            sugg = _esc(e.get("suggestion") or "(?)")
            out.append(Paragraph(
                f'<font color="#c0392b">{orig}</font> → <font color="#27ae60">{sugg}</font>',
                _T["gr_pair"],
            ))
            if e.get("explanation_vn"):
                out.append(Paragraph(_esc(e["explanation_vn"]), _T["gr_exp"]))

    tc, dc = gc.get("total_count"), gc.get("displayed_count")
    if isinstance(tc, int) and isinstance(dc, int) and tc > dc:
        out.append(Paragraph(f"+{tc - dc} lỗi khác đã được phát hiện.", _T["gr_more"]))
    out.append(Spacer(1, 6))
    return out
...
def _build_question_blocks(questions: list, responses_by_qid: dict) -> list:
    ...
    for i, q in enumerate(questions, start=1):
        ...
        # Structured grammar errors (LanguageTool) — mode-independent, sits below
        # the transcript (parity with result.html line 616).
        block.extend(_build_grammar_section(fb))
        ...
        # Pronunciation pills + phoneme drill-down — mode-independent, at the end
        # of the question block (parity with result.html _buildPronBlock).
        block.extend(_build_pronunciation_pills(response))
        block.extend(_build_phoneme_section(response))
