-- Migration 274 — bound migration 273's final-answer delivery envelope.
--
-- Staging review showed that the atomic envelope also needs a server-side
-- arrival bound: it rescues an eager request that transiently failed at zero,
-- but must not become an unlimited write path after the exam. Keep the same
-- 15-second grace used by the timeout reaper and replace the RPC in place.
...
BEGIN
    ...
    IF v_cutoff > v_now THEN
        RAISE EXCEPTION 'timed_course_finalize_not_expired'
            USING ERRCODE = '55000';
    END IF;
    -- This is a delivery envelope for answers already queued when the local
    -- canonical countdown reached zero, not an unlimited post-exam write path.
    -- The server reaper uses the same 15-second grace before taking ownership.
...
COMMENT ON FUNCTION public.quiz_finalize_timed_course_session(
    UUID, UUID, JSONB, JSONB
) IS 'Atomically stores a final batch within the 15-second canonical cutoff grace and closes the owned timed Course session.';
