-- Migration 297: canonical row-backed exam eligibility for the admin Review default.
-- Retakes have no shared exam-level section clock. Archived/draft exams can
-- retain review work. Eligibility follows persisted sitting AND review state.
               ...
               FROM (
                    SELECT DISTINCT exam.id
                     ...
                     WHERE exam.id = ANY(p_exam_ids)
                       AND (exam.exam_mode = 'retake' OR exam.status <> 'published')
...
'Backend-only actionable Review scope for retakes and unpublished exams with persisted review work; released and void sittings do not qualify.';
