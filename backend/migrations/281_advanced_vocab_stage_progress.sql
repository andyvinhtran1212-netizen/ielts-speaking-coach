...
-- Advanced Vocabulary banks are assignment-only. Keep the public browse
-- policy for all other banks, but do not disclose these bank identities to
-- anon/authenticated PostgREST clients. quiz_questions already has RLS enabled
-- with no SELECT policy, so its answer-bearing rows remain service-role only.
DROP POLICY IF EXISTS quiz_banks_public_read ON public.quiz_banks;
CREATE POLICY quiz_banks_public_read ON public.quiz_banks
    FOR SELECT
    USING (
        COALESCE(meta -> 'runtime' ->> 'kind', '') <> 'advanced_vocab'
    );
