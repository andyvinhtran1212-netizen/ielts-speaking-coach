-- Migration 266 — preserve timed Course retry semantics after migration 265.
--
-- `submitted_at` is immutable hand-in history, not a permanent write lock.
-- A failed attempt may still be entitled to revision/full retry while its
-- timer is live.  Only `passed_at` closes starts permanently.  When selecting
-- an open run session, stay inside the current full-retry generation.
...
DECLARE
    ...
    v_session_id UUID;
BEGIN
    SELECT cai.opened_at, cai.passed_at,
           NULLIF(cai.mastery ->> 'section_attempt_started_at', '')::TIMESTAMPTZ,
           ca.content_config, ca.status, ca.publish_at, ca.due_at
      INTO v_opened_at, v_passed_at, v_generation_started_at,
           v_config, v_status, v_publish_at, v_due_at
...
'Atomically authorizes timed Course starts, preserves failed-hand-in retries, and returns the current-generation open run session.';
