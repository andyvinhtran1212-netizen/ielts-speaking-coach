"""Static guards for migration 227's additive multi-class contract."""

from pathlib import Path


SQL = (Path(__file__).parents[1] / "migrations" /
       "227_student_cohort_memberships.sql").read_text(encoding="utf-8")
SQL_228 = (Path(__file__).parents[1] / "migrations" /
           "228_writing_idempotency_optional_cohort.sql").read_text(encoding="utf-8")
...
def test_followup_migration_restores_idempotent_individual_writing_gives():
    assert "IF v_cohort_id IS NOT NULL THEN" in SQL_228
    assert "IF v_cohort_id IS NULL THEN" not in SQL_228
    assert "student_id, cohort_id, assignment_group_id" in SQL_228
    assert "v_cohort_id, v_group_id" in SQL_228
    assert "REVOKE ALL ON FUNCTION" in SQL_228
    assert "TO service_role" in SQL_228
