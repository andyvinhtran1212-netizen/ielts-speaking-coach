...
def _fake_response(*, status_code: int, content: bytes = b"") -> MagicMock:
    ...
    r.content = content
    # raise_for_status mirrors requests.Response: only raises on 4xx/5xx.
    if status_code >= 400:
        def _raise():
...
def test_render_job_swallows_bucket_failure(monkeypatch):
    ...
    monkeypatch.setattr(
        listening_renderer.requests, "post",
        lambda *a, **kw: _fake_response(status_code=200, content=b"MP3"),
    )
