...
def _fresh_service_with_db(rows):
    ...
    db.table.return_value.select.return_value.order.return_value.range.return_value.execute.return_value = MagicMock(data=rows)
...
def test_db_unavailable_falls_back_to_markdown_G3():
    from services import vocab_content as vc
    db = MagicMock()
    db.table.return_value.select.return_value.order.return_value.range.return_value.execute.side_effect = RuntimeError("no db")
    with patch("database.supabase_admin", db):
        svc = vc.VocabContentService()
    assert svc._source == "markdown"
