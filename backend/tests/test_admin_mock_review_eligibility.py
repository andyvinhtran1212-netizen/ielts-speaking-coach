...
SQL = (Path(__file__).parents[1] / "migrations" /
       "297_admin_mock_review_eligibility.sql").read_text(encoding="utf-8")
...
def test_admin_exam_list_uses_persisted_retakes_and_stable_order(monkeypatch):
    db = _Database([
    ...
    ], actionable=["r-open", "a-sequential", "a-retake", "draft"])
    ...
    assert db.rpc_calls == [("fn_admin_mock_actionable_review_exam_ids", {
        "p_exam_ids": ["a-released", "a-retake", "a-sequential", "draft", "r-open", "r-released"],
    })]
    assert [row["id"] for row in rows] == ["s-open", "s-done", "r-released", "r-open", "draft", "a-sequential", "a-retake", "a-released"]
    assert {row["id"]: row["review_eligible"] for row in rows} == {
        "s-done": True, "r-open": True, "r-released": False,
        "s-open": False, "draft": True, "a-sequential": True,
...
def test_admin_exam_list_needs_no_retake_lookup_for_sequential_only(monkeypatch):
    db = _Database([_exam("s-done", "sequential", section="done")],
                   failure=RuntimeError("lookup unavailable"))
    monkeypatch.setattr(svc, "supabase_admin", db)
    assert svc.admin_list_exams()[0]["review_eligible"] is True
    assert db.rpc_calls == []
...
def test_admin_review_rpc_is_set_based_private_and_idempotent():
    ...
    assert "exam.exam_mode = 'retake' OR exam.status <> 'published'" in SQL
