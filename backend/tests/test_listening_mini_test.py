...
def _visibility_audio_or():
    ...
    return ("or", listening_mod._AUDIO_READY_PUBLIC_OR)
...
def test_list_mini_only():
    ...
    assert _or_clauses(rec) == [_visibility_audio_or()], "test_type must not be filtered via or_"
...
def test_list_filters_audio_readiness_in_sql_before_paging():
    ...
    """
    rec = _call(test_type="full")
    assert _visibility_audio_or() in rec
